...
import { api, type AppearanceSettings, type AppearanceStyle, type AppearanceTokens } from '@/api'
...
export const STYLE_BASELINES: Record<AppearanceStyle, Pick<AppearanceTokens, 'blur' | 'radius' | 'saturate'>> = {
...
export function applyAppearance(a: AppearanceSettings): void {
  ...
  s.setProperty('--bg-custom-light', a.customLight ? `url("${a.customLight}")` : 'none')
  s.setProperty('--bg-custom-dark', a.customDark ? `url("${a.customDark}")` : 'none')
}

export function useAppearance() {
  const query = useQuery({ queryKey: ['appearance'], queryFn: api.appearance.get, staleTime: 60_000 })
  const [override, setOverride] = useState<AppearanceStyle | null>(readOverride)
  ...
  useEffect(() => {
    ...
    const tokens = override ? { ...STYLE_BASELINES[override], accent: site.tokens.accent } : site.tokens
