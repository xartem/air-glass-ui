...
const dictionaries: Record<AdminLocale, Record<string, string>> = { ru, uk, en, de, fr, es, it, pl }

const STORAGE_KEY = 'admin.ui_locale'

let currentLocale: AdminLocale = (() => {
  const stored = localStorage.getItem(STORAGE_KEY)
  return ADMIN_LOCALES.includes(stored as AdminLocale) ? (stored as AdminLocale) : 'en'
})()
