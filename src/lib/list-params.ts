...
export function useListParams(): ListParams {
  ...
  const patch = (values: Record<string, string | undefined>, keepPage = false) => {
  ...
  return {
    ...
    setPage: (page) => patch({ page: page > 1 ? String(page) : undefined }, true),
    sort: sortColumn ? { column: sortColumn, dir: sortDir === 'desc' ? 'desc' : 'asc' } : undefined,
