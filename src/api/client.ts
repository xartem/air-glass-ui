...
const API_BASE = (import.meta.env.VITE_API_BASE as string | undefined) ?? '/console/api'
...
async function parseError(response: Response): Promise<ApiError> {
  let payload: { message?: string; code?: string; fields?: Record<string, string> } = {}
  try {
  ...
  if (response.status === 422) return new ValidationError(message, payload.fields ?? {})
...
export async function apiFetch<T>(path: string, options: RequestOptions = {}): Promise<T> {
  ...
  if (method !== 'GET') headers['X-CSRF-Token'] = readCookie('XSRF-TOKEN') ?? ''
...
export async function apiStream(path: string, { body, onEvent, signal }: StreamOptions): Promise<void> {
  if (useMock) {
