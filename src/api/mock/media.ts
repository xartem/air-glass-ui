...
export function startRegenerate(group?: string): {
  job_id: number;
  total: number;
} {
  // Rebuild variants for one owner group, or all presets when no group is given.
  const presets = group
    ? MEDIA_PRESETS.filter((preset) => preset.owner === group)
    : MEDIA_PRESETS;
...
export function regenerateStatus(jobId: number): RegenStatus {
  ...
  const processed = Math.min(
    job.total,
    Math.round((elapsed / REGEN_DURATION_MS) * job.total),
  );
  const state: RegenStatus["state"] =
    processed >= job.total ? "done" : "running";
  return {
    processed,
    total: job.total,
