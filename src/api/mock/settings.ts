...
const SCHEMA: { group: string; entries: SettingSchemaEntry[] }[] = [
  ...
  },
  {
    group: "content",
    entries: [
      { key: "content.per_page", type: "number", default: 12, has_help: true },
    ],
  },
  {
    group: "media",
    entries: [
      ...
      { key: "media.max_upload_mb", type: "number", default: 25 },
      {
        ...
        type: "text",
        default: "jpg,jpeg,png,webp,svg,gif,pdf",
        has_help: true,
      },
      { key: "media.quality", type: "number", default: 82, has_help: true },
      {
        key: "media.max_original_width",
        type: "number",
        default: 2560,
        has_help: true,
      },
  ...
  },
  {
    ...
    entries: [
      ...
      {
        key: "debug.slow_query_ms",
        type: "number",
        default: 150,
        has_help: true,
      },
      {
        key: "debug.sentry_dsn",
        type: "text",
        sensitive: true,
        default: "",
        has_help: true,
  ...
  },
  {
    ...
    entries: [
      ...
      {
        key: "ai.api_key.openai-compatible",
        type: "text",
        sensitive: true,
        default: "",
        section: "provider",
      ...
      {
        key: "ai.allowed_categories.read",
        type: "boolean",
        default: true,
        section: "categories",
      },
      {
        key: "ai.allowed_categories.write",
        type: "boolean",
        default: true,
        section: "categories",
      },
      {
        key: "ai.allowed_categories.destructive",
        type: "boolean",
        default: false,
        section: "categories",
        has_help: true,
      ...
      {
        key: "ai.findings_retention_days",
        type: "number",
        default: 90,
        section: "triage",
        has_help: true,
...
export function mediaPresetOverrides(): Record<string, MediaPresetOverride> {
  try {
    return JSON.parse(
      String(readStore(STORE_KEY)["media.preset_sizes"] ?? "{}"),
    );
  } catch {
    return {};
  }
  ...
  return {
    groups: SCHEMA.map(({ group, entries }) => ({
      ...
      values: Object.fromEntries(
        entries.map((entry) => {
          if (entry.sensitive)
            return [entry.key, sensitiveStored[entry.key] ? "***" : ""];
...
export function saveSettingsGroup(
  ...
  for (const [key, value] of Object.entries(changed)) {
    ...
    const entry = groupSchema.entries.find(
  ...
  if ("site.maintenance_mode" in changed) {
    localStorage.setItem(
      "mock.maintenance",
      String(changed["site.maintenance_mode"]),
    );
  }
  return { ok: true };
}
