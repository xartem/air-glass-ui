...
function roleHasImpersonate(roleId: number): boolean {
  return (
    rolesStore()
      ...
      ?.permissions.includes("users.impersonate") ?? false
  );
}
...
function toListItem(user: MockUser): UserListItem {
  ...
  return {
    ...
    impersonatable:
      !isSelf && user.is_active && !roleHasImpersonate(user.role_id),
    is_self: isSelf,
...
function assertEmailFree(email: string, exceptId?: number): void {
  const taken = usersStore().some(
    (user) =>
      user.email.toLowerCase() === email.toLowerCase() && user.id !== exceptId,
  );
  if (taken)
    throw new ValidationError("Validation failed", { email: "email_taken" });
}
...
export function createUser(
  body: Partial<MockUser> & { password?: string },
): UserDetail {
...
export function updateUser(
  ...
  const fields: Record<string, string> = {};
  const email =
    body.email !== undefined ? String(body.email).trim() : user.email;
  ...
  if (
    wasSoleAdmin &&
    ((demotingRole && Number(body.role_id) !== adminRoleId()) || deactivating)
  ) {
  ...
  if (!active) {
    if (id === currentUserId())
      throw new ValidationError("Validation failed", {
        _error: "self_deactivate",
      });
    const admins = activeAdminIds();
    if (admins.length === 1 && admins[0] === id)
      throw new ValidationError("Validation failed", { _error: "last_admin" });
  }
...
export function mfaBeginEnroll(
  userId: number,
  email: string,
...
export function mfaConfirmEnroll(
  userId: number,
  code: string,
...
export function mfaRegenerateCodes(userId: number): {
  recovery_codes: string[];
} {
  const store = mfaStore();
  const state = store[String(userId)];
...
export function changeProfilePassword(body: Record<string, unknown>): {
  ok: true;
} {
