    ...
    conversations: [
      {
        ...
        messages: [
          ...
          },
          {
            ...
            parts: [
              { kind: 'text', text: "Checked the draft — everything's ready. Here's the plan:" },
              { kind: 'plan', plan: executedPlan },
              { kind: 'tool', call: { tool: 'catalog.records.create', status: 'done', params: { title: 'Oslo Sofa' }, result: { id: 214 } } },
              { kind: 'tool', call: { tool: 'catalog.reindex', status: 'done', params: {}, result: { records: 148 } } },
              { kind: 'text', text: 'Done — the product is created and the catalog index is up to date.' },
              { kind: 'block', block: { type: 'link', label: 'Open product', route: '/shop/products/214' } },
            ],
      ...
      },
      {
        ...
        messages: [
          ...
          },
          {
            ...
            parts: [
              { kind: 'text', text: 'In the last 7 days you received 18 orders — 12% more than the previous week. Breakdown by channel:' },
              {
                kind: 'block',
                block: {
                  type: 'table',
              ...
              { kind: 'block', block: { type: 'link', label: 'Open orders', route: '/shop/orders' } },
            ],
...
const planScenario: Scenario = async (reply, _ctx, emit, signal) => {
  await emitText(reply, emit, signal, "Got it. Before I change anything — here's the plan:")
  await tick(signal, 300)
  const plan: AiPlan = {
    id: nextId(),
    description: 'Create the Oslo Sofa product: price $1,290, category Furniture, status draft.',
    steps: [
      { tool: 'catalog.records.create', summary: 'Create the product record' },
      { tool: 'media.attach', summary: 'Attach a cover image from the media library' },
...
const deleteScenario: Scenario = async (reply, _ctx, emit, signal) => {
  await emitText(reply, emit, signal, 'Deletion is irreversible, so I need your confirmation:')
...
const blocksScenario: Scenario = async (reply, _ctx, emit, signal) => {
  ...
  await emitBlock(reply, emit, signal, {
    ...
    items: [
      {
        entity_type: 'record', id: 7, title: 'Wireless Keyboard', subtitle: '$1,250 · published',
        preview_url: previewSvg('#bfdbfe'), badges: ['in stock'], route: '/shop/products/7',
        public_url: 'https://demo.example.com/products/wireless-keyboard',
      },
      {
        entity_type: 'record', id: 9, title: 'USB-C Hub', subtitle: '$890 · draft',
        preview_url: previewSvg('#bbf7d0'), badges: ['draft'], route: '/shop/products/9',
      },
  ...
  await emitBlock(reply, emit, signal, { type: 'file', media_id: 10, label: 'Price list', name: 'price-list.pdf', size: 482304 })
  await emitBlock(reply, emit, signal, { type: 'progress', job_id: 7, label: 'Catalog reindex', percent: 60 })
  await emitBlock(reply, emit, signal, {
    type: 'buttons',
    items: [
      { label: 'All products', kind: 'route', route: '/shop/products', style: 'primary' },
      { label: 'Storefront catalog', kind: 'url', url: 'https://demo.example.com/products', style: 'secondary' },
      { label: 'API documentation', kind: 'url', url: 'https://developers.example.dev/catalog', external: true, style: 'secondary' },
      { label: 'Delete USB-C Hub draft', kind: 'tool', tool: 'catalog.records.delete', args: { id: 9 }, style: 'danger' },
    ],
  })
  await emitBlock(reply, emit, signal, {
    type: 'link', label: 'Supplier price list', url: 'https://partner-warehouse.example.org/price', external: true,
  })
...
const limitScenario: Scenario = async (reply, _ctx, emit, signal) => {
  ...
  await emitText(reply, emit, signal, "The daily AI spend limit has been reached — let's continue tomorrow. An administrator can change the limit in settings.")
...
export async function streamAiPlanApprove(planId: number, emit: Emit, signal?: AbortSignal): Promise<void> {
  ...
  try {
    await emitToolRun(reply, emit, signal, 'catalog.records.create', { title: 'Oslo Sofa' }, { id: 214 })
    await emitToolRun(reply, emit, signal, 'media.attach', { record_id: 214, media: '2026/07/catalog-tile.webp' }, { ok: true })
    await emitToolRun(reply, emit, signal, 'catalog.records.update', { id: 214, price: 1290, category: 'Furniture' }, { ok: true })
    await emitText(reply, emit, signal, "Done: the product was created as a draft. Review the card and publish it when you're ready.")
    await emitBlock(reply, emit, signal, { type: 'link', label: 'Open Oslo Sofa', route: '/shop/products/214' })
...
export function rejectAiPlan(planId: number): { ok: true } {
  ...
  appendAssistant(found.conversation, [{ kind: 'text', text: 'Plan cancelled — I did not run anything.' }])
  return { ok: true }
}
...
export function confirmAiAction(id: number): { ok: true; tool: string; params: Record<string, string> } {
  ...
  appendAssistant(
    ...
    [
      ...
      { kind: 'text', text: 'Done. The action was recorded in the activity log, marked as AI.' },
    ],
...
export function cancelAiAction(id: number): { ok: true } {
  ...
  appendAssistant(found.conversation, [{ kind: 'text', text: 'Cancelled — I did not change anything.' }])
  return { ok: true }
}
  ...
  return [
    {
      ...
      summary: 'Concurrent writes to SQLite during checkout — likely a long transaction in OrderService holding the lock. Priority: high.',
    ...
    },
    {
      ...
      summary: 'The SMTP provider is not responding — emails fall into the retry queue and delivery is delayed.',
    ...
    },
    {
      ...
      summary: 'Mass 404s from old catalog URLs — a 301 redirect to the new structure is needed.',
