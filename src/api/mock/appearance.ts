...
function readStore(): AppearanceSettings {
  try {
    ...
    return {
      ...
      contentWidth: CONTENT_WIDTHS.includes(
...
export function saveAppearance(body: unknown): { ok: true } {
  ...
  if (input.style !== undefined && !STYLES.includes(input.style))
    fields.style = "Unknown style";
  if (input.dir !== undefined && !DIRS.includes(input.dir))
    fields.dir = "Unknown direction";
  if (input.bgLight !== undefined && !BGS.includes(input.bgLight))
    fields.bgLight = "Unknown background";
  if (input.bgDark !== undefined && !BGS.includes(input.bgDark))
    fields.bgDark = "Unknown background";
  if (input.density !== undefined && !DENSITIES.includes(input.density))
    fields.density = "Unknown density";
  if (
  ...
  if (input.bgLight === "custom" && !input.customLight)
    fields.customLight = "Pick an image";
  if (input.bgDark === "custom" && !input.customDark)
    fields.customDark = "Pick an image";
  ...
  const next: AppearanceSettings = {
    ...
    tokens: {
      ...
      saturate: clamp(
        Math.round(t.saturate ?? current.tokens.saturate),
        100,
        220,
      ),
