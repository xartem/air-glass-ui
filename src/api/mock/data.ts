...
export const ALL_PERMISSIONS = [
  'pages.view', 'pages.manage', 'pages.modals',
  'collections.view', 'collections.fields',
  'collections.pages.view', 'collections.pages.manage',
  'collections.blog.view', 'collections.blog.manage',
  'collections.products.view', 'collections.products.manage',
  ...
  'settings.view', 'settings.manage', 'modules.manage',
  'i18n.view', 'i18n.manage',
  'users.view', 'users.manage', 'roles.manage', 'users.impersonate',
  'mail.view', 'mail.manage',
  'notifications.channels',
  'scheduler.view', 'scheduler.manage',
  'cache.view', 'cache.manage',
  'backups.view', 'backups.manage', 'backups.restore',
  'updates.view', 'updates.manage',
  'activity.view', 'activity.restore',
  'dashboard.manage',
  'ai.use', 'ai.manage',
]
...
export const COLLECTION_LABELS: Record<string, Record<AdminLocale, string>> = {
  blog: { ru: 'Блог', uk: 'Блог', en: 'Blog', de: 'Blog', fr: 'Blog', es: 'Blog', it: 'Blog', pl: 'Blog' },
  products: { ru: 'Товары', uk: 'Товари', en: 'Products', de: 'Produkte', fr: 'Produits', es: 'Productos', it: 'Prodotti', pl: 'Produkty' },
}

export function localizeCollections(collections: Me['collections'], locale: AdminLocale): Me['collections'] {
...
export const MOCK_USERS: Record<MockUserKey, Me> = {
  admin: {
    user: { id: 1, name: 'Анна Админова', email: 'admin@demo.test', role: { key: 'admin', label: 'Администратор' }, ui_locale: 'ru' },
  ...
  editor: {
    user: { id: 2, name: 'Евгений Редактор', email: 'editor@demo.test', role: { key: 'editor', label: 'Редактор' }, ui_locale: 'ru' },
  ...
  viewer: {
    user: { id: 3, name: 'Виктор Новенький', email: 'viewer@demo.test', role: { key: 'viewer', label: 'Наблюдатель' }, ui_locale: 'ru' },
    permissions: [],
    locales: LOCALES,
    collections: [],
    impersonator: null,
...
const ENTITY_TYPES = ['page', 'record', 'form', 'menu', 'media', 'user', 'settings'] as const
const ACTIONS = ['created', 'updated', 'published', 'deleted', 'restored', 'reordered'] as const

const SAMPLE_TITLES = [
  'Главная страница', 'О компании', 'Контакты', 'Насос ГНОМ-25', 'Кровельные работы',
  'Форма обратной связи', 'Меню шапки', 'hero.jpg', 'Статья: новинки июля', 'Прайс-лист',
]

/** Entity-appropriate field diff for the audit fixture — settings show real keys, not a generic title/status. */
function changesFor(entityType: string, title: string): Record<string, { old: string; new: string }> {
  if (entityType === 'settings') {
    return {
      'general.site_name': { old: 'Universal CMS', new: 'Universal CMS — Пермь' },
      'general.timezone': { old: 'UTC', new: 'Europe/Moscow' },
      'mail.from_email': { old: 'noreply@example.com', new: 'hello@example.com' },
    }
  }
  if (entityType === 'user') {
    return {
...
export function buildActivityFixture(): ActivityEntry[] {
  ...
  for (let i = 0; i < 34; i++) {
    ...
    rows.push({
      ...
      changes: hasChanges ? changesFor(entityType, SAMPLE_TITLES[i % SAMPLE_TITLES.length]!) : null,
      url: entityType === 'page' ? '/pages/12' : entityType === 'record' ? '/c/blog/7' : null,
    })
  }
  return rows
}
...
export const ROLE_SEED: MockRole[] = [
  { id: 1, key: 'admin', label: 'Администратор', is_system: true, permissions: [...ALL_PERMISSIONS] },
  { id: 2, key: 'editor', label: 'Редактор', is_system: false, permissions: [...EDITOR_PERMISSIONS] },
  { id: 3, key: 'viewer', label: 'Наблюдатель', is_system: false, permissions: [] },
]
...
const USER_NAMES = [
  'Ольга Петрова', 'Дмитрий Соколов', 'Мария Кузнецова', 'Игорь Волков', 'Наталья Смирнова',
  'Павел Морозов', 'Елена Новикова', 'Сергей Козлов', 'Татьяна Лебедева', 'Андрей Попов',
  'Юлия Егорова', 'Максим Орлов', 'Ирина Фролова', 'Роман Зайцев', 'Светлана Титова',
  'Артём Белов', 'Оксана Гусева', 'Никита Тарасов', 'Вера Комарова', 'Алексей Крылов',
  'Людмила Сорокина',
]
...
export function buildUsersFixture(): MockUser[] {
  const users: MockUser[] = [
    { id: 1, name: 'Анна Админова', email: 'admin@demo.test', role_id: 1, ui_locale: 'ru', is_active: true, last_login_at: new Date(Date.now() - 20 * 60 * 1000).toISOString() },
    { id: 2, name: 'Евгений Редактор', email: 'editor@demo.test', role_id: 2, ui_locale: 'ru', is_active: true, last_login_at: new Date(Date.now() - 3 * 3600 * 1000).toISOString() },
    { id: 3, name: 'Виктор Новенький', email: 'viewer@demo.test', role_id: 3, ui_locale: 'ru', is_active: true, last_login_at: null },
  ]
  const roleCycle = [2, 2, 3, 2, 1, 2, 3, 2, 2, 3, 2, 2, 3, 2, 2, 3, 2, 2, 3, 2, 2]
  USER_NAMES.forEach((name, index) => {
    ...
    users.push({
      ...
      is_active: active,
      last_login_at: active && index % 4 !== 0 ? new Date(Date.now() - (index + 1) * 11 * 3600 * 1000).toISOString() : null,
    })
  })
  return users
}
...
export const SEARCH_GROUPS: AdminSearchGroup[] = [
  ...
  },
  {
    ...
    label: 'Заявки',
    items: [
      { title: '№432 — «нужен насос для дачи…»', url: '/forms/submissions?id=432' },
      { title: '№431 — «перезвоните по кровле»', url: '/forms/submissions?id=431' },
    ],
  },
]
