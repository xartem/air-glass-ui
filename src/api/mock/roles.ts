...
export function createRole(body: CreateRolePayload): RoleDetail {
  ...
  else if (rolesStore().some((role) => role.key === key))
    fields.key = "key_taken";
  if (!label) fields.label = "required";
  if (Object.keys(fields).length > 0)
    throw new ValidationError("Validation failed", fields);

  const store = rolesStore();
  const source = body.copy_from_id
    ? store.find((role) => role.id === body.copy_from_id)
    : undefined;
  const role: MockRole = {
...
export function renameRole(id: number, label: string): { ok: true } {
  ...
  if (!trimmed)
    throw new ValidationError("Validation failed", { label: "required" });
  role.label = trimmed;
  persist();
  return { ok: true };
}
...
export function deleteRole(id: number): { ok: true } {
  ...
  if (!role) throw new ApiError(404, "Role not found");
  if (role.is_system)
    throw new ApiError(422, "role_is_system", "role_is_system");
  if (usersOnRole(id) > 0)
    throw new ApiError(422, "role_has_users", "role_has_users");
