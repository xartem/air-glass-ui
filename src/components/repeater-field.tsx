...
function RepeaterRow({
  ...
  return (
    ...
    >
      ...
      <Button variant="ghost" size="icon-sm" aria-label={t('common.delete')} onClick={onRemove}>
...
export function RepeaterField<T>({
  ...
  while (keysRef.current.length < value.length) keysRef.current.push(`row-${counter.current++}`)
  if (keysRef.current.length > value.length) keysRef.current = keysRef.current.slice(0, value.length)
  ...
  return (
    <div data-slot="repeater-field" className={cn('space-y-2', className)}>
        ...
        <SortableContext items={keys} strategy={verticalListSortingStrategy}>
          <div className="space-y-2">
            {value.map((item, index) => (
              <RepeaterRow key={keys[index]} id={keys[index]!} onRemove={() => removeAt(index)}>
      ...
      <Button variant="outline" size="sm" onClick={() => onChange([...value, newItem()])}>
        <Plus />
