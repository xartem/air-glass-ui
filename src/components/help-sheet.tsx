...
const HelpArticleBody = lazy(() =>
  import('@/components/help-article').then((m) => ({ default: m.HelpArticleBody })),
)
    ...
    <>
      <Tooltip>
        <TooltipTrigger asChild>
          <Button variant="ghost" size="icon" aria-label={t('help.contextOpen')} onClick={() => setOpen(true)}>
            <CircleHelp />
          </Button>
        </TooltipTrigger>
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="right" className="flex w-full flex-col gap-0 sm:max-w-[480px]">
          <SheetHeader className="border-b border-border pb-3">
            ...
            <SheetTitle className="text-lg tracking-tight">{article.title}</SheetTitle>
          </SheetHeader>
          <div className="min-h-0 flex-1 overflow-y-auto px-4 py-2">
            <Suspense fallback={<div className="flex justify-center py-8"><Spinner className="size-5" /></div>}>
              <HelpArticleBody markdown={article.markdown} isFallback={article.is_fallback} />
            </Suspense>
          </div>
          <div className="flex justify-end border-t border-border px-4 py-3">
            <Button variant="link" asChild onClick={() => setOpen(false)}>
              <Link to={`/help/${article.module}/${article.page}`}>{t('help.openFull')} →</Link>
            </Button>
          </div>
        </SheetContent>
      </Sheet>
