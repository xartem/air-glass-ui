...
import { ArrowLeftRight, Copy, Droplet, Moon, Palette, RotateCcw, Sparkles, Square, Sun } from 'lucide-react'
import { toast } from 'sonner'

import { api, type AppearanceContentWidth, type AppearanceDensity, type AppearanceDir, type AppearanceSettings, type AppearanceStyle } from '@/api'
...
const ACCENT_PRESETS = ['#176dbd', '#7c3aed', '#0f766e', '#c2410c', '#be123c', '#4338ca']
...
function Segmented<T extends string>({
  ...
  return (
    <div className="space-y-2">
      ...
      <div role="group" aria-label={label} className="grid auto-cols-fr grid-flow-col gap-1 rounded-xl bg-muted/50 p-1">
        {options.map((opt) => (
          <button
            key={opt.value}
            type="button"
...
export function ThemeCustomizer({
  ...
  const query = useQuery({ queryKey: ['appearance'], queryFn: api.appearance.get, staleTime: 60_000 })
  ...
  useEffect(() => () => { if (savedRef.current) applyAppearance(savedRef.current) }, [])
  ...
  const patch = (next: Partial<AppearanceSettings>) => setDraft((d) => (d ? { ...d, ...next } : d))
  const setAccent = (accent: string) => setDraft((d) => (d ? { ...d, tokens: { ...d.tokens, accent } } : d))
  ...
  const selectStyle = (style: AppearanceStyle) => {
    onClearStyleOverride?.()
    setDraft((d) => (d ? { ...d, style, tokens: { ...STYLE_BASELINES[style], accent: d.tokens.accent } } : d))
  }
  const reset = () => {
    onClearStyleOverride?.()
          ...
          <div className="scrollbar-hover flex-1 space-y-5 overflow-y-auto p-4">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">{t('customizer.accent')}</span>
                <span className="text-xs text-muted-foreground tabular-nums">{draft.tokens.accent}</span>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <label
                  ...
                  style={{ background: 'conic-gradient(from 90deg, #ef4444, #f59e0b, #22c55e, #06b6d4, #6366f1, #ec4899, #ef4444)' }}
                >
                  <input
                    type="color"
            ...
            <Segmented
              ...
              options={STYLES.map((key) => {
                ...
                return { value: key, label: t(`customizer.style.${key}`), icon: <Icon className="size-4" /> }
              })}
            />
            ...
            <Segmented
              ...
              options={[
                { value: 'light', label: t('customizer.theme.light'), icon: <Sun className="size-4" /> },
                { value: 'dark', label: t('customizer.theme.dark'), icon: <Moon className="size-4" /> },
              ]}
            />
            ...
            <Segmented
              ...
              options={CONTENT_WIDTHS.map((key) => ({ value: key, label: t(`customizer.width.${key}`) }))}
            />
            ...
            <Segmented
              ...
              options={DIRS.map((key) => ({
                ...
                icon: <ArrowLeftRight className={cn('size-4', key === 'rtl' && '-scale-x-100')} />,
              }))}
            />
            ...
            <Segmented
              ...
              options={DENSITIES.map((key) => ({ value: key, label: t(`customizer.density.${key}`) }))}
            />
          </div>
        ) : (
