...
export function MediaRegenStatus({ job, onDone }: { job: RegenJob | null; onDone: () => void }) {
  const status = useQuery({
    queryKey: ['media-regen', job?.jobId],
    queryFn: () => api.media.regenerate.status(job!.jobId),
    enabled: job !== null,
    refetchInterval: (query) => (query.state.data?.state === 'done' ? false : 500),
  })
  ...
  const running = job !== null && status.data !== undefined && status.data.state !== 'done'

  return (
    <Panel icon={RefreshCw} title={t('media.regen.title')} description={t('media.regen.hint')}>
      {running && status.data ? (
        <div className="space-y-2">
          <p className="text-sm font-medium">{t('media.regen.running_group', { group: job!.group })}</p>
