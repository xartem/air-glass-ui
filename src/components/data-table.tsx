...
export function DataTable<TData>({
    ...
    onRowSelectionChange: (updater) => {
      ...
      if (onSelectionChange) {
        const selectedRows = data.filter(
          (_, index) => next[getRowId ? getRowId(data[index]!) : String(index)],
        );
  ...
  const resolvedState: DataTableState =
    state === "ready" && data.length === 0 ? "empty" : state;

  const handleSortClick = (columnId: string) => {
    if (!onSort) return;
    const nextDir =
      sort?.column === columnId && sort.dir === "asc" ? "desc" : "asc";
    ...
    <div data-slot="data-table" className={cn("flex flex-col", className)}>
      ...
      {resolvedState === "error" ? (
        <EmptyState
          ...
          action={
            onRetry ? { label: t("common.retry"), onClick: onRetry } : undefined
          }
        />
      ) : null}
