...
export function RoleMatrix({
  ...
  function groupCheckState(role: RoleDetail, keys: string[]): boolean | 'indeterminate' {
    if (role.is_system) return true
  ...
  function toggleGroup(role: RoleDetail, keys: string[], checked: boolean): void {
    if (role.is_system) return
    const current = new Set(value[role.id] ?? [])
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full border-separate border-spacing-0 text-sm">
        <thead>
          <tr>
            <th className="sticky top-0 left-0 z-20 bg-[var(--glass-bg)] px-3 py-2.5 text-left align-bottom backdrop-blur">
            ...
            {roles.map((role) => (
              <th
                key={role.id}
                className={cn('sticky top-0 z-10 bg-[var(--glass-bg)] px-3 py-2.5 align-bottom backdrop-blur', cellWidth)}
              >
                ...
                <div className="flex min-h-8 items-center justify-between gap-1">
                  <div className="min-w-0 truncate font-semibold">{role.label}</div>
                  {renderRoleMenu ? <div className="shrink-0">{renderRoleMenu(role)}</div> : null}
                </div>
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {groups.map(({ group, keys }) => (
            <GroupRows
              ...
              onToggleCollapse={() => setCollapsed((current) => ({ ...current, [group]: !isCollapsed(group) }))}
              has={has}
              toggle={toggle}
...
function GroupRows({
  ...
  groupCheckState: (role: RoleDetail, keys: string[]) => boolean | 'indeterminate'
  ...
  return (
    <>
      <tr className="border-t bg-muted/30">
        <th className="sticky left-0 z-10 bg-[var(--glass-bg)] px-3 py-2 text-left backdrop-blur">
          <button
            type="button"
        ...
        {roles.map((role) => (
          <td key={role.id} className={cn('px-3 py-2 text-center align-middle', cellWidth)}>
            <Checkbox
              checked={groupCheckState(role, keys)}
              disabled={role.is_system}
              aria-label={`${groupLabel} · ${role.label}`}
              onCheckedChange={(checked) => toggleGroup(role, keys, checked === true)}
            />
          </td>
        ))}
      </tr>
      {!collapsed
        ? keys.map((key) => (
            <tr key={key} className="border-t hover:bg-muted/20">
              <td className="sticky left-0 z-10 bg-[var(--glass-bg)] px-3 py-1.5 pl-9 backdrop-blur">
                <span className="font-mono text-xs text-muted-foreground">{key}</span>
              </td>
              {roles.map((role) => (
                <td key={role.id} className={cn('px-3 py-1.5 text-center align-middle', cellWidth)}>
                  <Checkbox
