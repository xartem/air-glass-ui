...
vi.mock("@/api", () => ({
  api: {
    auth: {
      relogin: (...args: unknown[]) => relogin(...args),
      challenge2fa: (...args: unknown[]) => challenge2fa(...args),
    },
  },
...
describe("ReloginDialog 2FA flow", () => {
  ...
  it("switches to the code step on mfa_required and resumes only after the challenge", async () => {
    ...
    await user.type(
      document.getElementById("relogin-password") as HTMLElement,
      "password",
    );
    ...
    await waitFor(() =>
      expect(document.getElementById("relogin-mfa")).toBeTruthy(),
    );
    expect(resumeAfterRelogin).not.toHaveBeenCalled();

    await user.type(
      document.getElementById("relogin-mfa") as HTMLElement,
      "123456",
    );
    await user.click(submit());
