...
import { api, type MediaPreset, type MediaPresetOverride, type MediaPresetMode } from '@/api'
...
function effective(preset: MediaPreset, override: MediaPresetOverride): MediaPreset {
...
export function MediaPresetsTable({
  ...
  const presetsQuery = useQuery({ queryKey: ['media-presets'], queryFn: api.media.presets })
  ...
  const saved = useMemo(() => presetsQuery.data?.overrides ?? {}, [presetsQuery.data])
  ...
  useEffect(() => {
    ...
    for (const preset of presets) {
      const merged: MediaPresetOverride = { ...saved[preset.key], ...edits[preset.key] }
  ...
  function patch(key: string, override: MediaPresetOverride) {
    setEdits((current) => ({ ...current, [key]: { ...current[key], ...override } }))
  }

  if (presetsQuery.isPending) {
    return (
      <Panel icon={Image} title={t('media.presets.title')} description={t('media.presets.hint')}>
    ...
    return (
      <Panel icon={Image} title={t('media.presets.title')}>
        <EmptyState
          ...
          action={{ label: t('common.retry'), onClick: () => void presetsQuery.refetch() }}
        />
      </Panel>
    )
  }
  ...
  for (const preset of presets) {
    ...
    else owners.push({ owner: preset.owner, is_theme: preset.is_theme, rows: [preset] })
  }
  ...
  return (
    ...
    >
      <Table>
        ...
        <TableBody>
          {owners.map((group) => [
            <TableRow key={`owner-${group.owner}`} className="bg-muted/40 hover:bg-muted/40">
              <TableCell colSpan={6} className="py-1.5">
                <div className="flex items-center justify-between gap-2">
                  <span className="inline-flex items-center gap-2 text-xs font-semibold tracking-wide uppercase">
                    {group.owner}
                    {group.is_theme ? (
                      <Badge variant="secondary">{t('media.presets.theme_badge')}</Badge>
                    ) : null}
                  </span>
                  ...
                  {canManage && onRegenerateGroup ? (
                    <Button
                      ...
                      size="sm"
                      className="h-7"
                      onClick={() => onRegenerateGroup(group.owner, group.rows.length)}
                    >
            ...
            ...group.rows.map((preset) => {
              const row = effective(preset, { ...saved[preset.key], ...edits[preset.key] })
              return (
                <TableRow key={preset.key}>
                  <TableCell className="font-mono text-xs">{preset.key}</TableCell>
                  <TableCell className="text-muted-foreground uppercase">{preset.format}</TableCell>
                  <TableCell>
                    <Select
                      value={row.mode}
                      onValueChange={(mode) => patch(preset.key, { mode: mode as MediaPresetMode })}
                    >
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1.5">
                      <Input
                        ...
                        onChange={(event) => patch(preset.key, { width: Number(event.target.value) })}
                      />
                      ...
                      <Input
                        ...
                        onChange={(event) => patch(preset.key, { height: Number(event.target.value) })}
                      />
                    </div>
                  </TableCell>
                  <TableCell>
                    <Input
                      ...
                      className="h-8 w-24"
                      value={row.quality ?? ''}
                      placeholder={t('media.presets.quality_placeholder', { value: globalQuality })}
                      onChange={(event) =>
                        patch(preset.key, {
                          quality: event.target.value === '' ? null : Number(event.target.value),
                        })
                      }
                    />
                  </TableCell>
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <span className="inline-flex">
                          <Switch
                            checked={row.retina}
                            onCheckedChange={(checked) => patch(preset.key, { retina: checked === true })}
                            aria-label={t('media.presets.col.retina')}
                          />
                        </span>
                      </TooltipTrigger>
                      <TooltipContent>{t('media.presets.retina_hint')}</TooltipContent>
                    </Tooltip>
                  </TableCell>
                </TableRow>
              )
