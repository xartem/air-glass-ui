...
import { Area, AreaChart, Bar, BarChart, CartesianGrid, ReferenceLine, XAxis } from 'recharts'

import type { ChartData, ListData, ListItem, StatData, StatusData, StatusRow, WidgetSize } from '@/api'
...
import { MASONRY_GAP, MASONRY_ROW, MasonryContext } from '@/components/widget-grid'
...
function Sparkline({
  ...
  return (
    <ChartContainer
      ...
      className={cn(
        'aspect-auto',
        fullWidth ? '-mx-[var(--card-spacing)] w-[calc(100%_+_2*var(--card-spacing))]' : 'w-24 shrink-0',
      )}
      style={{ height }}
    ...
    >
      <AreaChart data={data} margin={{ top: 3, right: 0, bottom: 3, left: 0 }}>
        ...
        {labels ? <ChartTooltip cursor={false} content={<SparklineTooltip />} /> : null}
        <Area
          dataKey="y"
          type="monotone"
          stroke={`url(#${strokeId})`}
...
function StatCompare({ compare }: { compare: NonNullable<StatData['compare']> }) {
...
function StatGoal({ value, goal }: { value: number; goal: NonNullable<StatData['goal']> }) {
  const pct = goal.target > 0 ? Math.min(100, Math.round((value / goal.target) * 100)) : 0
  return (
    <div className="mt-2">
...
function StatDelta({ delta }: { delta: number }) {
  return (
    <p className="mt-1.5 flex items-center gap-1.5 text-xs">
      ...
      <span className="text-muted-foreground">{t('dashboard.delta_period')}</span>
    </p>
  )
}
...
export function StatCard({
  ...
  const value =
    typeof data.value === 'number' ? <CountUpValue value={data.value} /> : data.value
  ...
  const extras = (
    <>
      ...
      {data.goal && numericValue !== undefined ? <StatGoal value={numericValue} goal={data.goal} /> : null}
    </>
  )
    ...
    return (
      <WidgetCardFrame title={title} href={href} icon={icon} className={className}>
        <div className="flex flex-col gap-1.5">
          <p className="text-5xl leading-none font-semibold tracking-tight tabular-nums">{value}</p>
          {extras}
          {data.series ? (
            <Sparkline points={data.series} labels={data.series_labels} height={40} fullWidth />
          ) : null}
        </div>
      </WidgetCardFrame>
    )
    ...
    return (
      <WidgetCardFrame title={title} href={href} icon={icon} className={className}>
        <div className="flex flex-col gap-2">
          <p className="text-5xl leading-none font-semibold tracking-tight tabular-nums">{value}</p>
          {extras}
          <Sparkline points={data.series} labels={data.series_labels} height={120} fullWidth />
        </div>
      </WidgetCardFrame>
    )
  }
    ...
    <WidgetCardFrame title={title} href={href} icon={icon} className={className}>
      <div className="flex flex-col">
        <div className="flex items-end justify-between gap-2">
          ...
          {data.series ? (
            <Sparkline points={data.series} labels={data.series_labels} height={expanded ? 44 : 32} />
          ) : null}
        </div>
        {extras}
      </div>
...
export const CHART_HEIGHT: Record<WidgetSize, number> = { sm: 64, md: 180, lg: 220, xl: 260 }
...
export function ChartCard({
    ...
    () =>
      Object.fromEntries(
        data.series.map((entry, index) => [
          `s${index}`,
          { label: t(entry.label_key), color: `var(--chart-${(index % 5) + 1})` },
        ]),
      ),
    [data.series],
  )
  ...
  const dashedKeys = new Set(seriesMeta.filter((entry) => entry.dashed).map((entry) => entry.key))
      ...
      >
        {data.kind === 'bar' ? (
          <BarChart data={rows} margin={{ top: 4, right: 4, bottom: 0, left: 4 }}>
            <defs>
              {seriesKeys.map((key) => (
                <linearGradient key={key} id={`${gradientBase}-${key}`} x1="0" y1="0" x2="0" y2="1">
            ...
            {size === 'xl' && seriesKeys.length > 1 ? <ChartLegend content={<ChartLegendContent />} /> : null}
            ...
            {seriesKeys.map((key) =>
              dashedKeys.has(key) ? (
                ...
                <Bar key={key} dataKey={key} fill={`url(#${gradientBase}-${key})`} radius={[4, 4, 0, 0]} maxBarSize={28} />
              ),
            )}
          </BarChart>
        ) : (
          <AreaChart data={rows} margin={{ top: 4, right: 4, bottom: 0, left: 4 }}>
            <defs>
              {seriesKeys.map((key) => (
                <linearGradient key={key} id={`${gradientBase}-${key}`} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor={`var(--color-${key})`} stopOpacity={0.2} />
            ...
            {seriesKeys.map((key) => (
              <Area
                ...
                fill={dashedKeys.has(key) ? 'transparent' : `url(#${gradientBase}-${key})`}
                dot={false}
...
/** Shared with the widget skeletons (rigid skeleton contract, E4/E6). */
export const LIST_ROWS: Record<WidgetSize, number> = { sm: 2, md: 6, lg: 8, xl: 10 }
...
function ListRow({ item, rich }: { item: ListData['items'][number]; rich: boolean }) {
  ...
  const body = (
    <>
      ...
      {item.hint ? (
        <span className="min-w-0 shrink truncate text-xs text-muted-foreground">{item.hint}</span>
      ) : null}
      ...
      {item.badge ? (
        ...
        <Badge variant="secondary" className={cn(item.badge === 'new' && 'bg-primary/10 text-primary')}>
  ...
  return <li className={cn('flex items-center gap-2 text-sm', rich && 'py-1.5')}>{body}</li>
}
...
function WidgetEmpty() {
  return (
    <div className="flex flex-col items-center justify-center gap-1.5 py-4 text-center">
      ...
      <p className="text-sm text-muted-foreground">{t('dashboard.widget_empty')}</p>
    </div>
  )
}
...
export function ListCard({
  ...
  const viewAll =
    href ? (
      <Link to={href} className="mt-3 inline-flex items-center gap-1 text-sm text-primary hover:underline">
...
function StateIcon({ state }: { state: StatusRow['state'] }) {
  if (state === 'error') return <CircleX className="size-4 shrink-0 text-destructive" />
  if (state === 'warn')
    return <TriangleAlert className="size-4 shrink-0" style={{ color: 'var(--status-pending-fg)' }} />
  return <CircleCheck className="size-4 shrink-0" style={{ color: 'var(--status-success-fg)' }} />
}
    ...
    <>
      <span className="flex min-w-0 flex-1 items-center gap-2">
        ...
        <span
          className={cn(
            'truncate',
            size === 'sm' ? 'text-base text-foreground' : 'text-muted-foreground',
          )}
        >
          {t(row.label_key)}
        </span>
      ...
      <StatusBadge status={STATE_TO_STATUS[row.state]} label={row.value} className="min-w-0 max-w-[50%]" />
    </>
  )
  if (row.url) {
    return (
  ...
  return (
    <WidgetCardFrame title={title} href={href} icon={icon} className={className}>
      {rows.length === 0 ? (
        ...
        <ul className={cn(size === 'lg' || size === 'xl' ? 'space-y-2.5' : 'space-y-1.5')}>
          {rows.map((row) => (
