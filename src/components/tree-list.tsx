...
function findParent(nodes: TreeNode[], id: string, parent: TreeNode | null = null): { parent: TreeNode | null; index: number } | null {
...
function cloneTree(nodes: TreeNode[]): TreeNode[] {
  return nodes.map((node) => ({ ...node, children: node.children ? cloneTree(node.children) : undefined }))
}
...
function TreeRow({
  ...
  return (
    <div
      ref={setNodeRef}
      style={{ transform: CSS.Transform.toString(transform), transition, marginLeft: depth * 24 }}
      className={cn(
    ...
    >
      ...
      {hasChildren ? (
        <Button variant="ghost" size="icon-xs" onClick={onToggle} aria-expanded={!collapsed}>
          {collapsed ? <ChevronRight /> : <ChevronDown />}
        </Button>
      ) : (
        <span className="size-6" />
      )}
      <span className="min-w-0 flex-1 truncate text-sm">{renderLabel ? renderLabel(node) : node.label}</span>
      <span className="flex opacity-0 transition-opacity group-focus-within:opacity-100 group-hover:opacity-100">
        <Button variant="ghost" size="icon-xs" onClick={onOutdent} disabled={!canOutdent} aria-label={t('tree.outdent')}>
          <IndentDecrease />
        </Button>
        <Button variant="ghost" size="icon-xs" onClick={onIndent} disabled={!canIndent} aria-label={t('tree.indent')}>
          <IndentIncrease />
        </Button>
      </span>
    </div>
