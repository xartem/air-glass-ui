...
function AiPanelInner() {
  ...
  useEffect(() => {
    ...
    if (
      activeId !== null &&
      listQuery.data.some((conversation) => conversation.id === activeId)
    )
      return;
    setActive(listQuery.data[0]?.id ?? null);
  }, [open, listQuery.data, activeId]);

  const active = listQuery.data?.find(
    (conversation) => conversation.id === activeId,
  );
  const title =
    activeId === null ? t("ai.title") : (active?.title ?? t("ai.untitled"));
