...
export function HelpArticleBody({ markdown, isFallback, className }: {
  markdown: string
  isFallback?: boolean
  className?: string
}) {
  ...
  return (
    <div className={cn('min-w-0 text-[15px] leading-relaxed text-secondary-foreground', className)}>
      {isFallback ? (
        <p className="mb-4 text-[13px] text-muted-foreground italic">{t('help.fallbackNote')}</p>
      ) : null}
      <ReactMarkdown
        components={{
          h1: ({ children }) => <h2 className="mt-6 mb-2 text-lg font-semibold tracking-tight text-foreground">{children}</h2>,
          ...
          a: ({ href, children }) => (
            <a href={href} className="font-medium text-primary hover:underline">{children}</a>
          ),
          img: ({ src, alt }) => (
            <img
              src={src}
              ...
              onClick={() => src && setLightbox({ src: String(src), alt: alt ?? '' })}
            />
          ),
          blockquote: ({ children }) => (
      ...
      <Dialog open={lightbox !== null} onOpenChange={(open) => !open && setLightbox(null)}>
        <DialogContent className="max-w-4xl p-2 sm:p-3">
          <DialogTitle className="sr-only">{lightbox?.alt || t('help.screenshot')}</DialogTitle>
          {lightbox ? (
            <img src={lightbox.src} alt={lightbox.alt} className="max-h-[80vh] w-full rounded-lg object-contain" />
          ) : null}
        </DialogContent>
      </Dialog>
    </div>
