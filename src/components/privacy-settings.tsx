...
const TEXT_FIELDS: (keyof ConsentText)[] = [
  "title",
  "description",
  "accept",
  "reject",
...
function parseTexts(raw: SettingValue): Record<string, ConsentText> {
  try {
    ...
    return parsed && typeof parsed === "object"
      ? (parsed as Record<string, ConsentText>)
      : {};
  } catch {
    return {};
  }
...
function usePrivacyValues() {
  ...
  const privacy = settingsQuery.data?.groups.find(
    (group) => group.group === "privacy",
  );
...
export function PrivacyCategories({
  ...
  if (pending) {
    return (
      <Panel
        icon={Cookie}
        title={t("privacy.categories.title")}
        description={t("privacy.categories.hint")}
      >
  ...
  return (
    ...
    >
      ...
      <Button
        ...
        className="mt-3"
        onClick={() =>
          setCategories((current) => [...current, { key: "", required: false }])
        }
      >
        <Plus />
...
export function PrivacyTexts({
  ...
  const { privacy, pending } = usePrivacyValues();
  const initial = useMemo(
    () => parseTexts(privacy?.values["privacy.consent_texts"] ?? null),
    [privacy],
  );
  ...
  useEffect(() => {
    ...
    onOverridesChange(
      json === JSON.stringify(cleanTexts(initial)) ? null : json,
    );
  ...
  if (pending) {
    return (
      <Panel
        icon={Cookie}
        title={t("privacy.texts.title")}
        description={t("privacy.texts.hint")}
      >
    ...
    >
      <LanguageTabs
        locales={SITE_LOCALES.map((code) => ({
          code,
          state:
            Object.keys(cleanTexts({ [code]: texts[code] ?? {} })).length > 0
              ? "translated"
              : "missing",
        }))}
      >
        {(locale) => (
          <div className="space-y-3">
            <div className="space-y-1.5">
              <Label htmlFor={`consent-title-${locale}`}>
                {t("privacy.text.banner_title")}
              </Label>
              <Input
                ...
                onChange={(event) =>
                  setText(locale, "title", event.target.value)
                }
              />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor={`consent-desc-${locale}`}>
                {t("privacy.text.description")}
              </Label>
              <Textarea
                ...
                onChange={(event) =>
                  setText(locale, "description", event.target.value)
                }
              />
            </div>
            <div className="grid gap-3 sm:grid-cols-3">
              {(["accept", "reject", "settings"] as const).map((field) => (
                <div key={field} className="space-y-1.5">
                  <Label htmlFor={`consent-${field}-${locale}`}>
                    {t(`privacy.text.${field}`)}
                  </Label>
                  <Input
                    ...
                    onChange={(event) =>
                      setText(locale, field, event.target.value)
                    }
                  />
                </div>
              ))}
