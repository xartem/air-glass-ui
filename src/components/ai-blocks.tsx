import { Ban, Download, ExternalLink, FileText, Image as ImageIcon, Play, Trash2 } from 'lucide-react'
...
const BUTTON_VARIANT: Record<AiBlockButton['style'], 'default' | 'outline' | 'destructive'> = {
  primary: 'default',
  secondary: 'outline',
  danger: 'destructive',
}
...
function LinkBlock({ block }: { block: Extract<AiBlock, { type: 'link' }> }) {
  ...
  return (
    <a href={block.url} target="_blank" rel="noopener" className={className}>
      ...
      {block.external ? <ExternalLink aria-hidden className="size-3.5" /> : null}
    </a>
  )
}
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {items.map((item, index) => {
        ...
        return (
          <Button key={index} size="sm" variant={variant} onClick={() => onToolClick?.(item)}>
...
function CardItem({ card }: { card: AiCardView }) {
  if (card.stub) {
    return (
      <div className="flex items-center gap-2.5 rounded-xl border border-dashed bg-background/40 px-3 py-2.5 text-sm text-muted-foreground">
        ...
        {t(card.stub === 'deleted' ? 'ai.blocks.deleted' : 'ai.blocks.no_access')}
      </div>
    )
  }
  const body = (
    <>
      {card.preview_url ? (
        <img src={card.preview_url} alt="" className="size-11 shrink-0 rounded-lg border object-cover" />
      ) : (
  ...
  return (
    <div className="flex items-center gap-2.5 rounded-xl border bg-background/60 p-2.5">
      {card.route ? (
        <Link to={card.route} className="flex min-w-0 flex-1 items-center gap-2.5 hover:opacity-80">
          {body}
        </Link>
      ) : (
      ...
      {card.public_url ? (
        <Button variant="ghost" size="icon-sm" aria-label={t('ai.blocks.open_site')} asChild>
...
function ImageBlock({ block }: { block: Extract<AiBlock, { type: 'image' }> }) {
  ...
  return (
    <Link to="/media" className="inline-block" aria-label={t('ai.blocks.open_media')}>
      {block.preview_url ? (
        <img src={block.preview_url} alt={block.alt ?? ''} className="max-h-40 max-w-full rounded-xl border" />
      ) : (
...
function VideoBlock({ block }: { block: Extract<AiBlock, { type: 'video' }> }) {
  return (
    <Link
      ...
      style={block.poster_url ? { backgroundImage: `url(${block.poster_url})`, backgroundSize: 'cover' } : undefined}
    >
      <span className="flex size-10 items-center justify-center rounded-full bg-background/80">
        <Play className="size-4" />
      </span>
      {block.label ? (
        <span className="absolute inset-x-0 bottom-0 truncate bg-background/80 px-2 py-1 text-xs">{block.label}</span>
      ) : null}
    </Link>
  )
}
...
function FileBlock({ block }: { block: Extract<AiBlock, { type: 'file' }> }) {
  return (
    <div className="flex max-w-full items-center gap-2.5 rounded-xl border bg-background/60 p-2.5">
      ...
      <span className="min-w-0 flex-1">
        <span className="block truncate text-sm font-medium">{block.label ?? block.name}</span>
...
function ProgressBlock({ block }: { block: Extract<AiBlock, { type: 'progress' }> }) {
  return (
