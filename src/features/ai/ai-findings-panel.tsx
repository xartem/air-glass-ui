...
const SEVERITY_BADGE: Record<AiFindingSeverity, StatusKind> = {
  critical: "error",
  error: "pending",
};
const STATUS_BADGE: Record<AiFindingStatus, StatusKind> = {
  new: "info",
          ...
          {finding.status !== "resolved" ? (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem onClick={() => onResolve(finding)}>
                  {t("ai.findings.resolve")}
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          ) : null}
        ...
        <CollapsibleContent>
          <div className="space-y-2 pb-3 text-xs">
            <p className="text-muted-foreground">
              {t("ai.findings.first_seen")}: {formatTime(finding.first_seen)} ·{" "}
              {t("ai.findings.last_seen")}: {formatTime(finding.last_seen)}
            </p>
            <p className="font-medium">{t("ai.findings.sample")}</p>
            <pre className="overflow-x-auto rounded-lg border bg-muted/40 p-2 font-mono">
              {finding.sample}
            </pre>
            ...
            {finding.resolved_note ? (
              <p className="text-muted-foreground">
                {t("ai.findings.resolved_note", {
                  note: finding.resolved_note,
                })}
              </p>
            ) : null}
          </div>
...
export function AiFindingsPanel() {
  ...
  function submitResolve() {
    if (resolveTarget)
      statusMutation.mutate({ id: resolveTarget.id, status: "resolved", note });
      ...
      actions={
        <>
          ...
          >
            <SelectTrigger
              size="sm"
              aria-label={t("ai.findings.filter.severity")}
            >
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
          ...
          >
            <SelectTrigger
              size="sm"
              aria-label={t("ai.findings.filter.status")}
            >
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
          ...
          {rows.map((finding) => (
            <FindingRow
              ...
              onAcknowledge={(target) =>
                statusMutation.mutate({ id: target.id, status: "acknowledged" })
              }
              onResolve={(target) => {
                setNote("");
