...
export function LoginPage() {
  ...
  function fail(cause: unknown) {
    if (cause instanceof ApiError && (cause.status === 429 || cause.code === 'throttled')) {
      setThrottled(true)
    } else if (cause instanceof ValidationError) {
      setFieldError(t(mfaStep ? 'login.invalid_code' : 'login.invalid_credentials'))
    } else {
    ...
    <Card>
      ...
      <CardContent>
        ...
        <form onSubmit={submit} className="space-y-4" noValidate>
          ...
          <FormField name="password" label={t('login.password')} required error={fieldError ?? undefined}>
