...
const ENTITY_TYPES = ['page', 'record', 'form', 'menu', 'media', 'user', 'settings']
const ACTIONS = ['created', 'updated', 'published', 'deleted', 'restored', 'reordered']
...
export function ActivityPage() {
  ...
  const [restoreTarget, setRestoreTarget] = useState<ActivityEntry | null>(null)
  ...
  const searchActors = useCallback(async (query: string): Promise<ReferenceItem[]> => {
    const result = await api.users.list({ q: query || undefined, page: 1 })
    return result.rows.slice(0, 10).map((user) => ({ id: user.id, label: user.name, hint: user.email }))
  }, [])
  ...
  const actorValue = useMemo<ReferenceItem | null>(() => {
    ...
    return { id: Number(actorId), label: actorLookup.data?.name ?? `#${actorId}` }
    ...
    () => [
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <span className="whitespace-nowrap text-muted-foreground">
                  {formatDistanceToNow(new Date(iso), { addSuffix: true, locale: DATE_LOCALES[locale] })}
                </span>
              </TooltipTrigger>
        ...
        cell: ({ row }) => {
          ...
          return (
            <span className="flex min-w-48 items-baseline gap-1.5">
              <span className="font-medium whitespace-nowrap">{actionLabel(entry.action)}</span>
              <span className="truncate text-muted-foreground">{entityTitle(entry)}</span>
            </span>
          )
        },
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return entry.url ? (
            <Link to={entry.url} className="whitespace-nowrap text-primary underline-offset-2 hover:underline">
              {label}
            </Link>
          ) : (
    ...
    <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            <label className="flex items-center gap-2 text-sm">
              <Switch
                ...
                onCheckedChange={(checked) => patchParams({ ai: checked ? '1' : undefined })}
              />
            ...
            <Select
              ...
              onValueChange={(value) => patchParams({ type: value === 'all' ? undefined : value })}
            >
              <SelectTrigger className="w-44">
                <SelectValue placeholder={t('activity.filter.type')} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t('activity.filter.all_types')}</SelectItem>
            ...
            <Select
              ...
              onValueChange={(value) => patchParams({ action: value === 'all' ? undefined : value })}
            >
              <SelectTrigger className="w-44">
                <SelectValue placeholder={t('activity.filter.action')} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t('activity.filter.all_actions')}</SelectItem>
                {ACTIONS.map((action) => (
      ...
      >
        <DataTable<ActivityEntry>
          ...
          emptyState={{ title: t('activity.empty'), description: t('activity.empty_hint') }}
        />
      </Panel>

      {/* Details: full metadata + the field · old · new diff from `changes` */}
      <Sheet open={details !== null} onOpenChange={(open) => !open && setDetails(null)}>
          ...
          {details ? (
            <div className="space-y-5 px-4 pb-4">
              <dl className="grid grid-cols-[7rem_1fr] gap-x-3 gap-y-2 text-sm">
                <dt className="text-muted-foreground">{t('activity.detail.actor')}</dt>
                <dd className="flex flex-wrap items-center gap-1.5">
                  ...
                  {details.impersonator ? (
                    <span className="inline-flex items-center gap-1 text-xs text-muted-foreground">
                      ...
                      {t('activity.detail.impersonated_by', { name: details.impersonator.name })}
                    </span>
                  ) : null}
                </dd>

                <dt className="text-muted-foreground">{t('activity.detail.action')}</dt>
                <dd>
                  <Badge variant="outline">{actionLabel(details.action)}</Badge>
                </dd>

                <dt className="text-muted-foreground">{t('activity.detail.entity')}</dt>
                <dd>
                  {details.url ? (
                    <Link
                      to={details.url}
                ...
                <dt className="text-muted-foreground">{t('activity.detail.date')}</dt>
                <dd>{siteTime.formatLong(details.created_at)}</dd>
              </dl>

              {details.changes ? (
                <div className="space-y-2">
                  <p className="text-sm font-medium">{t('activity.detail.changes')}</p>
                  {/* Field · was → became, stacked so long values stay readable in the sheet */}
                  <ul className="space-y-2">
                    {Object.entries(details.changes).map(([field, change]) => (
                      <li key={field} className="rounded-lg p-2.5 ring-1 ring-border">
                        <p className="font-mono text-xs break-all text-muted-foreground">{field}</p>
                ...
                <p className="text-sm text-muted-foreground">{t('activity.no_changes')}</p>
              )}
            </div>
          ) : null}
        </SheetContent>
      ...
      <ConfirmDialog
        ...
        description={t('activity.restore_description', {
          fields: restoreTarget?.changes ? Object.keys(restoreTarget.changes).join(', ') : '',
        })}
