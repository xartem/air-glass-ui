...
function highlight(text: string, needle: string) {
  ...
  return (
    <>
      {text.slice(0, idx)}
      <mark className="rounded-sm bg-primary/15 px-0.5 text-inherit">{text.slice(idx, idx + needle.length)}</mark>
...
function TreeNav({
  ...
  const activeGroupKey = groups.find((g) => g.articles.some((a) => a.module === active.module && a.page === active.page))?.key
...
function SearchResults({ q, onNavigate }: { q: string; onNavigate?: () => void }) {
  const query = useQuery({ queryKey: ['help-search', q], queryFn: () => api.help.search(q) })
  ...
  if (!query.data?.length) {
    return <EmptyState icon={SearchX} title={t('help.emptyTitle')} description={t('help.emptyHint')} className="py-8" />
  }
  return (
    <div className="flex flex-col gap-px">
      {query.data.map((hit) => (
        ...
        >
          <div className="text-[14px] font-medium text-foreground">{highlight(hit.title, q)}</div>
          <div className="mt-0.5 line-clamp-2 text-[12.5px] text-muted-foreground">{highlight(hit.snippet, q)}</div>
          <div className="mt-0.5 text-[11.5px] text-muted-foreground/70">{hit.section}</div>
        </Link>
      ))}
    </div>
  )
...
function ArticlePane({ module, page }: { module: string; page: string }) {
  ...
  if (query.isError || !query.data) {
    return (
      <EmptyState
        ...
        action={{ label: t('help.backToSections'), onClick: () => navigate('/help') }}
      />
    )
  }
  ...
  return (
    <article>
      ...
      <h2 className="text-[24px] leading-tight font-semibold tracking-tight text-balance">{article.title}</h2>
      ...
      <HelpArticleBody markdown={article.markdown} isFallback={article.is_fallback} className="mt-2" />
...
export function HelpPage() {
  ...
  const treeQuery = useQuery({ queryKey: ['help-tree'], queryFn: api.help.tree, staleTime: Infinity })
  ...
  const sidePanelContent = (
    <div className="flex flex-col gap-3">
      ...
      {searching ? (
        ...
        <TreeNav groups={groups} active={{ module, page }} onNavigate={() => setMobileNavOpen(false)} />
      )}
    </div>
  )
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <p className="-mt-3 text-sm text-muted-foreground">{t('help.subtitle')}</p>
      ...
      <div className="lg:hidden">
        ...
        <Sheet open={mobileNavOpen} onOpenChange={setMobileNavOpen}>
          <SheetContent side="left" className="w-full gap-0 sm:max-w-[320px]">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-3 pb-4">{sidePanelContent}</div>
          </SheetContent>
        </Sheet>
      </div>

      <div className="flex items-start gap-4">
        <Panel className="sticky top-4 hidden w-[280px] shrink-0 lg:block" contentClassName="p-3.5">
          {sidePanelContent}
        </Panel>
