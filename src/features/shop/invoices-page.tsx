...
export function InvoicesPage() {
  ...
  const filters = {
    ...
    sort:
      (params.sort?.column as "number" | "issued_at" | "amount" | undefined) ??
      "issued_at",
  ...
  const columns = useMemo<ColumnDef<InvoiceListItem>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="whitespace-nowrap text-muted-foreground">
            {format(new Date(row.original.issued_at), "PP", {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="whitespace-nowrap text-muted-foreground">
            {format(new Date(row.original.due_at), "PP", {
              locale: DATE_LOCALES[locale],
            })}
          </span>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <StatusBadge
            ...
            label={t(`shop.invoices.status.${row.original.status}`)}
          />
        ),
      },
      {
    ...
    <div className="space-y-4">
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="all">
                  {t("shop.invoices.filter.all_statuses")}
                </SelectItem>
      ...
      >
        <DataTable<InvoiceListItem>
          ...
          emptyState={{
            title: t("shop.invoices.empty"),
            description: t("shop.invoices.empty_hint"),
          }}
        />
      </Panel>
    </div>
