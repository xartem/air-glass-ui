...
import { ArrowLeft, Mail, MapPin, Phone, ShoppingBag, UserCircle } from 'lucide-react'
...
const STATUS_KIND = { active: 'success', vip: 'info', blocked: 'error' } as const
    ...
    return (
      <div className="space-y-4">
        <PageHeader
          ...
          icon={UserCircle}
          breadcrumbs={[{ label: t('nav.customers'), href: '/shop/customers' }, { label: t('shop.customers.detail.title') }]}
        />
        <Panel>
          <EmptyState
            title={t('table.error.title')}
            description={t('table.error.description')}
            action={{ label: t('common.retry'), onClick: () => void customerQuery.refetch() }}
          />
        </Panel>
      </div>
    )
        ...
        <>
          <div className="grid gap-4 lg:grid-cols-3">
            <Panel
              ...
              actions={<StatusBadge status={STATUS_KIND[customer.status]} label={t(`shop.customers.status.${customer.status}`)} />}
            >
              <dl className="space-y-3 text-sm">
                ...
                <div className="flex items-start gap-2">
                  ...
                  <span className="whitespace-pre-line text-muted-foreground">{customer.address}</span>
                </div>
                <div className="text-xs text-muted-foreground">
                  {t('shop.customers.detail.joined', {
                    date: format(new Date(customer.joined_at), 'PP', { locale: DATE_LOCALES[locale] }),
                  })}
                </div>
              </dl>
            </Panel>
            ...
            <div className="grid gap-4 sm:grid-cols-3 lg:col-span-2 lg:content-start">
              <StatTile label={t('shop.customers.detail.orders')} value={String(customer.orders_count)} />
              <StatTile label={t('shop.customers.detail.spend')} value={formatMoney(customer.ltv, customer.currency, locale)} />
              <StatTile label={t('shop.customers.detail.aov')} value={formatMoney(customer.aov, customer.currency, locale)} />

              <div className="sm:col-span-3">
                <Panel icon={ShoppingBag} title={t('shop.customers.detail.recent_orders')}>
                  {customer.recent_orders.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t('shop.customers.detail.no_orders')}</p>
                  ) : (
                    <Table>
                      <TableHeader>
                        <TableRow>
              ...
              {customer.notes.map((note) => (
                <li key={note.id} className="flex gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {note.author} · {format(new Date(note.at), 'PPp', { locale: DATE_LOCALES[locale] })}
                    </div>
                  </div>
                </li>
              ))}
