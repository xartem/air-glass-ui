...
export function CustomersPage() {
  ...
  const filters = {
    ...
    status: (params.filter('status') as CustomerStatus | undefined) ?? undefined,
    sort: (params.sort?.column as 'name' | 'orders_count' | 'ltv' | 'joined_at' | undefined) ?? 'name',
  ...
  const columns = useMemo<ColumnDef<CustomerListItem>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <StatusBadge status={STATUS_KIND[row.original.status]} label={t(`shop.customers.status.${row.original.status}`)} />
        ),
      },
      {
        id: 'joined_at',
        ...
        cell: ({ row }) => (
          <span className="whitespace-nowrap text-muted-foreground">
            {format(new Date(row.original.joined_at), 'PP', { locale: DATE_LOCALES[locale] })}
          </span>
        ),
      },
    ],
  ...
  const rowActions: RowAction<CustomerListItem>[] = [
    ...
    },
    {
      ...
      onSelect: (customer) => {
        console.debug('[CustomersPage] impersonate (demo)', { id: customer.id })
    ...
    <div className="space-y-4">
        ...
        actions={
          <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="all">{t('shop.customers.filter.all_statuses')}</SelectItem>
      ...
      >
        <DataTable<CustomerListItem>
          ...
          emptyState={{ title: t('shop.customers.empty'), description: t('shop.customers.empty_hint') }}
        />
      </Panel>
    </div>
  )
