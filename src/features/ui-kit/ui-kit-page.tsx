...
import { type DataTableState, type PaginationMeta, type RowAction } from '@/components/data-table'
...
import { MediaPicker, MediaTile, type MediaItem, type MediaPage } from '@/components/media-picker'
import { PaginationBar } from '@/components/pagination-bar'
import { Modal } from '@/components/modal'
import { MultiSelect } from '@/components/multi-select'
import { PageHeader } from '@/components/page-header'
import { DateRangePicker, type DateRangeValue } from '@/components/date-range-picker'
import { ReferencePicker, type ReferenceItem } from '@/components/reference-picker'
...
import { ChartCard, ListCard, StatCard, StatusCard, WidgetCardFrame } from '@/components/widget-cards'
...
function PaletteSection() {
  ...
  const statuses: StatusKind[] = ['published', 'draft', 'pending', 'error', 'info', 'archived']
  return (
    <Section id="palette" title={t('uikit.section.palette')}>
      <div className="flex flex-wrap gap-3">
        {swatches.map((swatch) => (
          <div key={swatch.name} className="flex items-center gap-2 rounded-lg border px-3 py-2">
            <span className="size-6 rounded-md border" style={{ backgroundColor: `var(${swatch.variable})` }} />
            <code className="text-xs text-muted-foreground">{swatch.variable}</code>
          </div>
        ))}
      </div>
      <Separator />
    ...
    <Section id="forms" title={t('uikit.section.forms')}>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <div className="md:col-span-2 xl:col-span-3">
          <p className="mb-2 text-sm font-medium">{t('uikit.switchrow.title')}</p>
    ...
    <Section id="language" title={t('uikit.section.language')}>
      <div className="space-y-6">
        ...
        <div className="space-y-2">
          <LocaleSwitcher
            locales={[
              { code: 'ru', label: 'Русский', is_default: true, state: 'translated' },
              { code: 'en', label: 'English', is_default: false, state: 'stale' },
              { code: 'uk', label: 'Українська', is_default: false, state: 'missing' },
            ]}
            value={editorLocale}
            onChange={setEditorLocale}
          />
          <FormField name={`title-global-${editorLocale}`} label={`${t('uikit.field.title')} (${editorLocale})`} required>
        ...
        >
          {(locale) => (
            <FormField name={`title-${locale}`} label={`${t('uikit.field.title')} (${locale})`} required>
  ...
  return (
    <Section id="savebar" title={t('uikit.section.savebar')}>
      <div className="max-w-2xl space-y-3">
        <FormField name="savebar-demo" label={t('uikit.field.title')} help={t('uikit.savebar.hint')}>
          <Input id="savebar-demo" value={value} onChange={(event) => setValue(event.target.value)} />
        </FormField>
        <SaveBar
          dirty={dirty}
          saving={saving}
...
function loadDemoMedia(query: string, page: number): Promise<MediaPage> {
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      resolve({
        items: filtered.slice((safePage - 1) * MEDIA_PER_PAGE, safePage * MEDIA_PER_PAGE),
        pagination: { page: safePage, pages, total: filtered.length, perPage: MEDIA_PER_PAGE },
      })
    }, 300)
  })
}
...
function PickersSection() {
  ...
  const [range, setRange] = useState<DateRangeValue>({ from: '2026-06-01', to: '2026-06-30' })
  ...
  const searchReferences = (query: string) =>
    new Promise<ReferenceItem[]>((resolve) => {
      setTimeout(
        () => resolve(DEMO_REFERENCES.filter((item) => item.label.toLowerCase().includes(query.toLowerCase()))),
        300,
      )
    })
    ...
    <Section id="pickers" title={t('uikit.section.pickers')}>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <FormField
          ...
          className="md:col-span-2 xl:col-span-3"
        >
          <ReferencePicker id="picker-reference" value={reference} onChange={setReference} search={searchReferences} />
        </FormField>
        <FormField name="picker-media" label={t('uikit.field.media')} className="md:col-span-2 xl:col-span-3">
          <MediaPicker
            value={media}
            onChange={setMedia}
            multiple
    ...
    <Section id="repeater" title={t('uikit.section.repeater')}>
      <RepeaterField
        ...
        renderItem={(item, index, update) => (
          <div className="grid gap-2 sm:grid-cols-3">
            <FormField name={`repeater-label-${index}`} label={t('uikit.field.title')}>
              <Input
            ...
            <FormField name={`repeater-url-${index}`} label={t('uikit.field.url')}>
              <Input
            ...
            <FormField name={`repeater-icon-${index}`} label={t('uikit.field.icon')}>
              <Input
                id={`repeater-icon-${index}`}
                value={item.icon}
                onChange={(event) => update({ ...item, icon: event.target.value })}
              />
            </FormField>
          </div>
        )}
...
function ListArchetypeSection() {
  ...
  const rows = useMemo(
    () => DEMO_ROWS.filter((row) => row.title.toLowerCase().includes(search.toLowerCase())),
    [search],
  )
  ...
  const columns: ColumnDef<DemoRow, unknown>[] = useMemo(
    () => [
      ...
      { accessorKey: 'updatedAt', id: 'updatedAt', header: t('common.updated') },
    ],
    [],
  )
  ...
  return (
    <Section id="list" title={t('uikit.section.list')}>
      ...
      <ListLayout<DemoRow>
        title={t('nav.pages')}
        icon={FileText}
        primaryAction={{ label: t('common.add'), icon: <Plus />, onClick: () => toast(t('common.add')) }}
        search={{ value: search, onChange: setSearch }}
        filters={
          <Select value={state} onValueChange={(value) => setState(value as DataTableState)}>
            <SelectTrigger className="w-44">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="ready">{t('uikit.state.ready')}</SelectItem>
              <SelectItem value="loading">{t('uikit.state.loading')}</SelectItem>
  ...
  return (
    <Section id="overlays" title={t('uikit.section.overlays')}>
      <div className="flex flex-wrap items-center gap-2">
        <Modal
          ...
          footer={
            <>
              ...
              <Button onClick={() => setModalOpen(false)}>{t('common.save')}</Button>
            </>
          }
        >
        ...
        <Button variant="outline" onClick={() => toast.success(t('uikit.toastSaved'))}>
          {t('uikit.showToast')}
        </Button>
  ...
  return (
    <Section id="empty" title={t('uikit.section.empty')}>
      <EmptyState
        ...
        action={{ label: t('common.add'), onClick: () => toast(t('common.add')) }}
      />
      <Separator />
...
const WIDGET_DEMO_X = ['21.06', '22.06', '23.06', '24.06', '25.06', '26.06', '27.06', '28.06', '29.06', '30.06', '01.07', '02.07', '03.07', '04.07']
const WIDGET_DEMO_CHART = {
  kind: 'bar' as const,
  series: [
    {
      label_key: 'uikit.widget.orders_chart',
      points: [6, 9, 7, 12, 10, 14, 11, 16, 13, 18, 15, 20, 17, 22].map((y, index) => ({ x: WIDGET_DEMO_X[index]!, y })),
    },
    {
      // Second series demos the xl-only legend (D:dashboard §4 fullest tier).
      label_key: 'uikit.widget.leads',
      points: [3, 5, 4, 7, 6, 8, 5, 9, 7, 10, 8, 11, 9, 12].map((y, index) => ({ x: WIDGET_DEMO_X[index]!, y })),
    },
  ],
}
...
function WidgetsSection() {
  return (
    <Section id="widgets" title={t('uikit.section.widgets')}>
      ...
      <WidgetGrid>
        <StatCard title={t('uikit.widget.products')} icon="shopping-cart" size="sm" data={WIDGET_DEMO_STAT} className={WIDGET_SPAN.sm} />
        <StatCard title={t('uikit.widget.products')} icon="shopping-cart" size="md" data={WIDGET_DEMO_STAT} className={WIDGET_SPAN.md} />
        <StatCard title={t('uikit.widget.products')} icon="shopping-cart" size="lg" data={WIDGET_DEMO_STAT} className={WIDGET_SPAN.lg} />
        <StatCard title={t('uikit.widget.products')} icon="shopping-cart" size="xl" data={WIDGET_DEMO_STAT} className={WIDGET_SPAN.xl} />
      </WidgetGrid>
      <WidgetGrid>
        <ChartCard title={t('uikit.widget.orders_chart')} icon="chart-column" size="md" data={WIDGET_DEMO_CHART} className={WIDGET_SPAN.md} />
        <ChartCard title={t('uikit.widget.orders_chart')} icon="chart-column" size="lg" data={WIDGET_DEMO_CHART} className={WIDGET_SPAN.lg} />
        <ChartCard title={t('uikit.widget.orders_chart')} icon="chart-column" size="xl" data={WIDGET_DEMO_CHART} className={WIDGET_SPAN.xl} />
      </WidgetGrid>
      <WidgetGrid>
        <ListCard title={t('uikit.widget.leads')} icon="inbox" size="sm" data={WIDGET_DEMO_LIST} className={WIDGET_SPAN.sm} />
        <ListCard title={t('uikit.widget.leads')} icon="inbox" size="md" data={WIDGET_DEMO_LIST} className={WIDGET_SPAN.md} />
        <ListCard title={t('uikit.widget.leads')} icon="inbox" size="lg" data={WIDGET_DEMO_LIST} className={WIDGET_SPAN.lg} />
        <StatusCard title={t('uikit.widget.queue')} icon="timer" size="md" data={WIDGET_DEMO_STATUS} className={WIDGET_SPAN.md} />
      </WidgetGrid>
      <Separator />
      {/* Loading / error / empty contract of a widget card (E4 §4, UI:dashboard §2) */}
      <p className="text-xs text-muted-foreground">{t('uikit.widget.states')}</p>
      <WidgetGrid>
        <WidgetCardFrame title={t('uikit.widget.pages')} icon="file-text" className={WIDGET_SPAN.sm}>
          <WidgetSkeleton type="stat" size="sm" />
        </WidgetCardFrame>
        <WidgetCardFrame title={t('uikit.widget.orders_chart')} icon="chart-column" className={WIDGET_SPAN.sm}>
          <WidgetSkeleton type="chart" size="sm" />
        </WidgetCardFrame>
        <WidgetCardFrame title={t('uikit.widget.leads')} icon="inbox" className={WIDGET_SPAN.sm}>
        ...
        <ListCard title={t('uikit.widget.leads')} icon="inbox" size="sm" data={{ items: [] }} className={WIDGET_SPAN.sm} />
      </WidgetGrid>
    </Section>
  )
}
...
function EditorArchetypeSection() {
  ...
  return (
    <section id="editor" className="space-y-3">
      <h2 className="text-lg font-semibold tracking-tight">{t('uikit.section.editor')}</h2>
          ...
          <EditorLayout
            ...
            tabs={[
              ...
              },
              {
                ...
                icon: Search,
                content: (
                  <FormField name="editor-seo-title" label={t('uikit.field.title')}>
                    <Input id="editor-seo-title" onChange={() => setDirty(true)} />
                  </FormField>
                ),
              },
              {
                ...
                content: (
                  ...
                  >
                    {(locale) => (
                      <FormField name={`editor-title-${locale}`} label={`${t('uikit.field.title')} (${locale})`}>
                        <Input id={`editor-title-${locale}`} onChange={() => setDirty(true)} />
                      </FormField>
                    )}
                  </LanguageTabs>
                ),
...
function SettingsArchetypeSection() {
  ...
  return (
    <section id="settings-archetype" className="space-y-3">
      <h2 className="text-lg font-semibold tracking-tight">{t('uikit.section.settings')}</h2>
      <Card className="overflow-visible">
        <CardContent>
          ...
          >
            <div className="grid max-w-xl gap-4">
              {active === 'general' ? (
                <>
                  <FormField name="settings-site-name" label={t('uikit.field.title')} required>
                    <Input id="settings-site-name" defaultValue="Universal CMS" onChange={() => setDirty(true)} />
                  </FormField>
                  <FormField name="settings-site-url" label={t('uikit.field.url')}>
                    <Input id="settings-site-url" defaultValue="https://example.com" onChange={() => setDirty(true)} />
                  </FormField>
                </>
              ) : active === 'mail' ? (
                <FormField name="settings-mail-from" label={t('uikit.field.email')}>
                  <Input id="settings-mail-from" defaultValue="noreply@example.com" onChange={() => setDirty(true)} />
                </FormField>
              ) : (
                <FormField name="settings-locale" label={t('editor.tab.languages')}>
      ...
      <WizardDialog
        ...
        steps={[
          ...
          },
          {
            ...
            content: <p className="text-sm text-muted-foreground">{t('uikit.subtitle')}</p>,
          },
        ]}
      />
    </Section>
...
function CompositionSection() {
  ...
  return (
    <section id="composition" className="space-y-3">
      <h2 className="text-lg font-semibold tracking-tight">{t('uikit.section.composition')}</h2>
      <Card>
        <CardContent className="space-y-4">
          ...
          {Object.entries(regions).map(([regionKey, blocks]) => (
            <div key={regionKey} className="rounded-xl border border-dashed p-3">
                ...
                renderItem={(block) => {
                  ...
                  return (
                    <div className="flex items-center gap-2.5">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => setConfiguring(block)}>
...
function MenuBuilderSection() {
  ...
  return (
    <section id="menu-builder" className="space-y-3">
      <h2 className="text-lg font-semibold tracking-tight">{t('uikit.section.menuBuilder')}</h2>
      <Card>
        <CardContent>
          ...
          <div className="flex flex-col gap-4 lg:flex-row">
            ...
            <aside className="w-full shrink-0 space-y-3 rounded-xl border p-3 lg:w-72">
              <FormField name="menu-reference" label={t('reference.placeholder')}>
                <ReferencePicker
                  id="menu-reference"
                  value={reference}
              ...
              <Button
                ...
                onClick={() => {
                  if (!reference) return
                  setMenu([...menu, { id: `m-${reference.id}-${menu.length}`, label: reference.label }])
                  setReference(null)
                }}
              >
                <Plus />
...
function MediaGridSection() {
  ...
  const pages = Math.max(1, Math.ceil(filtered.length / MEDIA_PER_PAGE))
  const safePage = Math.min(page, pages)
  const items = filtered.slice((safePage - 1) * MEDIA_PER_PAGE, safePage * MEDIA_PER_PAGE)

  return (
    <section id="media-grid" className="space-y-3">
      <h2 className="text-lg font-semibold tracking-tight">{t('uikit.section.mediaGrid')}</h2>
      <Card>
...
function PermissionsMatrixSection() {
  const [value, setValue] = useState<RolePermissionMap>(() =>
    Object.fromEntries(MATRIX_DEMO_ROLES.map((role) => [role.id, role.permissions])),
  )

  return (
    <section id="permissions" className="space-y-3">
      <h2 className="text-lg font-semibold tracking-tight">{t('uikit.section.permissions')}</h2>
      <Card>
        <CardContent>
          ...
          <RoleMatrix
            ...
            value={value}
            onChange={(roleId, keys) => setValue((current) => ({ ...current, [roleId]: keys }))}
          />
        </CardContent>
      </Card>
    </section>
...
function ErrorsSection() {
  return (
    <section id="errors" className="space-y-3">
      <h2 className="text-lg font-semibold tracking-tight">{t('uikit.section.errors')}</h2>
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        ...
        <Card>
          <CardContent>
            <ErrorPage code="500" onRetry={() => toast.success(t('uikit.toastSaved'))} className="min-h-0 px-2 py-6" />
          </CardContent>
        </Card>
        <Card>
          <CardContent>
...
function LoginSection() {
  return (
    <section id="login" className="space-y-3">
      <h2 className="text-lg font-semibold tracking-tight">{t('uikit.section.login')}</h2>
      ...
      <div className="flex justify-center rounded-xl border border-dashed py-10">
        <Card className="w-full max-w-sm">
          ...
          <CardContent className="space-y-4">
            ...
            <FormField name="login-password" label={t('login.password')} required>
              <Input id="login-password" type="password" />
            </FormField>
            <Button className="w-full" onClick={() => toast.success(t('uikit.toastSaved'))}>
              <Lock className="size-4" />
              {t('login.submit')}
            </Button>
...
export function UiKitPage() {
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-8 pb-16">
      ...
      <p className="-mt-4 text-sm text-muted-foreground">{t('uikit.subtitle')}</p>
