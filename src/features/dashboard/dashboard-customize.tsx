...
import { api, type RoleDetail, type WidgetSize, type WidgetType, type LayoutOverrides } from '@/api'
...
export function DashboardCustomize({
  ...
  const roleLabel = roles.find((role) => role.key === roleKey)?.label ?? roleKey
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      ...
      visible.forEach((tile, index) => {
        overrides.widgets[tile.key] = { size: tile.size, sort: (index + 1) * 10, hidden: false }
      })
      hidden.forEach((tile, index) => {
        overrides.widgets[tile.key] = { size: tile.size, sort: (visible.length + index + 1) * 10, hidden: true }
      })
      visibleActions.forEach((action, index) => {
        overrides.actions![action.key] = { sort: (index + 1) * 10, hidden: false }
      })
      hiddenActions.forEach((action, index) => {
        overrides.actions![action.key] = { sort: (visibleActions.length + index + 1) * 10, hidden: true }
      })
  ...
  function onDragEnd(event: DragEndEvent) {
    ...
    setTiles((current) => {
      const visibleKeys = current.filter((tile) => !tile.hidden).map((tile) => tile.key)
  ...
  function onActionDragEnd(event: DragEndEvent) {
    ...
    if (!over || active.id === over.id) return
    setActions((current) => {
      const visibleKeys = current.filter((action) => !action.hidden).map((action) => action.key)
      const from = visibleKeys.indexOf(String(active.id))
      const to = visibleKeys.indexOf(String(over.id))
      if (from < 0 || to < 0) return current
      const nextVisible = arrayMove(
  ...
  function hideAction(key: string) {
    setActions((current) => current.map((action) => (action.key === key ? { ...action, hidden: true } : action)))
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-x-4 gap-y-2">
        <div>
          <h1 className="text-xl font-semibold tracking-tight">{t('dashboard.customize')}</h1>
          <p className="text-sm text-muted-foreground">
            {t('dashboard.customize.editing_role', { role: roleLabel })}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="ghost" onClick={onExit} disabled={saveMutation.isPending}>
            {t('common.cancel')}
          </Button>
      ...
      {query.isPending ? (
        <WidgetGrid>
          {Array.from({ length: 6 }, (_, index) => (
            <Skeleton key={index} className="h-20 rounded-xl md:col-span-3 xl:col-span-3" />
          ))}
        </WidgetGrid>
      ) : (
        <>
                  ...
                  <SortableContext items={visibleActions.map((action) => action.key)} strategy={rectSortingStrategy}>
                    <div className="flex flex-col gap-2">
                      {visibleActions.map((action) => (
                        <SortableActionRow key={action.key} action={action} onHide={() => hideAction(action.key)} />
                      ))}
                    </div>
                  </SortableContext>
                </DndContext>
          ...
          <Panel icon={EyeOff} title={t('dashboard.customize.hidden_title')}>
            {nothingHidden ? (
              <p className="text-sm text-muted-foreground">{t('dashboard.customize.hidden_empty')}</p>
            ) : (
              <ul className="space-y-2">
                {hidden.map((tile) => {
                  const Icon = dashboardIcon(tile.icon) ?? TYPE_ICONS[tile.type]
                  return (
                    <li key={tile.key} className="flex items-center gap-2 text-sm">
                      <Icon className="size-4 shrink-0 text-muted-foreground" />
                      <span className="min-w-0 flex-1 truncate">{t(tile.title_key)}</span>
                      <Button variant="outline" size="sm" onClick={() => restoreTile(tile.key)}>
                        <Eye />
                ...
                {hiddenActions.map((action) => {
                  ...
                  return (
                    <li key={action.key} className="flex items-center gap-2 text-sm">
                      <Icon className="size-4 shrink-0 text-muted-foreground" />
                      <span className="min-w-0 flex-1 truncate">{t(action.label_key)}</span>
                      <Button variant="outline" size="sm" onClick={() => restoreAction(action.key)}>
                        <Eye />
                        {t('dashboard.customize.restore')}
                      </Button>
                    </li>
          ...
          <div>
            <Button variant="outline" onClick={() => setResetOpen(true)} disabled={resetMutation.isPending}>
...
function SortableTile({
  ...
  return (
    <Card
      ...
      className={cn(WIDGET_SPAN[tile.size], isDragging && 'z-10 opacity-80 shadow-lg')}
    >
      <CardContent className="flex items-center gap-2">
        ...
        <span className="min-w-0 flex-1 truncate text-sm font-medium">{t(tile.title_key)}</span>
        <DropdownMenu>
        ...
        <Button variant="ghost" size="icon-sm" aria-label={t('dashboard.customize.hide')} onClick={onHide}>
          <EyeOff />
        </Button>
      </CardContent>
    </Card>
...
function SortableActionRow({
  ...
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: action.key })
  const Icon = dashboardIcon(action.icon) ?? Zap
  return (
    <Card
      ref={setNodeRef}
    ...
    >
      <CardContent className="flex items-center gap-2">
        ...
        <Icon className="size-4 shrink-0 text-primary" />
        <span className="min-w-0 flex-1 truncate text-sm font-medium">{t(action.label_key)}</span>
        <Button variant="ghost" size="icon-sm" aria-label={t('dashboard.customize.hide')} onClick={onHide}>
          <EyeOff />
        </Button>
      </CardContent>
