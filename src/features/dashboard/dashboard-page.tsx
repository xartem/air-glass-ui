...
export function DashboardPage() {
  ...
  const customizeRole =
    canManage &&
    customizeParam &&
    rolesQuery.data?.some((role) => role.key === customizeParam)
      ? customizeParam
      : null;
  ...
  return (
    <DashboardHome
      onCustomize={() => setSearchParams({ customize: me.user.role.key })}
    />
  );
}
    ...
    <div className="space-y-6">
      ...
      <WidgetGrid>
        {Array.from({ length: 4 }, (_, index) => (
          <Skeleton
            key={index}
            className="h-36 rounded-xl md:col-span-3 xl:col-span-3"
          />
        ))}
      </WidgetGrid>
    </div>
...
function DashboardHome({ onCustomize }: { onCustomize: () => void }) {
  ...
  const isEmpty =
    query.isSuccess && widgets.length === 0 && actions.length === 0;
    ...
    mutationFn: (action: DashboardAction) => {
      ...
      if (!run)
        return Promise.reject(
          new Error(`Unknown api action: ${action.api ?? ""}`),
        );
      return run();
    },
    onSuccess: (_result, action) => {
      if (action.key === "cache.clear")
        toast.success(t("dashboard.action.cache_cleared"));
    },
