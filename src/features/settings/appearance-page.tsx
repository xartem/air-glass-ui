...
function loadMedia(query: string, page: number): Promise<MediaPage> {
  return api.media.list(query, page).then((res) => ({
    items: res.rows.map((m) => ({ path: m.path, name: m.name, previewUrl: m.preview_url })),
    pagination: {
      page: res.page,
...
export function AppearancePage() {
  ...
  if (!draft) {
    return (
      <div className="space-y-4">
        ...
        <Panel><div className="h-40 animate-pulse rounded-lg bg-muted" /></Panel>
      </div>
    )
  }

  const patch = (next: Partial<AppearanceSettings>) => setDraft((d) => (d ? { ...d, ...next } : d))
  const setToken = (key: keyof AppearanceSettings['tokens'], value: number | string) =>
  ...
  return (
    <div className="space-y-4 pb-24">
      ...
      {/* Style */}
      <Panel icon={Palette} title={t('settings.appearance.style')} description={t('settings.appearance.style_desc')}>
      ...
      {/* Backgrounds */}
      <Panel icon={ImageIcon} title={t('settings.appearance.bg')} description={t('settings.appearance.bg_desc')}>
        <div className="space-y-6">
          {(['light', 'dark'] as const).map((theme) => {
            const active = theme === 'light' ? draft.bgLight : draft.bgDark
            const customValue = theme === 'light' ? draft.customLight : draft.customDark
            const setBg = (bg: AppearanceBg) => patch(theme === 'light' ? { bgLight: bg } : { bgDark: bg })
            const setCustom = (path: string | null) =>
              patch(theme === 'light' ? { customLight: path } : { customDark: path })
            return (
              <div key={theme} className="space-y-3">
                <p className="text-sm font-medium">{t(`settings.appearance.bg_${theme}`)}</p>
                  ...
                  {BG_KEYS.map((bg) => (
                    <button
                      ...
                      className={cn(
                        ...
                        active === bg ? 'border-primary ring-3 ring-ring/40' : 'hover:border-ring/60',
                      )}
                    >
                      <span
                        ...
                        style={bg === 'custom' ? undefined : { backgroundImage: BG_SWATCH[theme][bg] }}
                      >
                        {bg === 'custom' ? <ImageIcon className="size-4" /> : null}
                      </span>
                      <span className="text-xs">{t(`settings.appearance.bg.${bg}`)}</span>
                    </button>
                  ))}
                </div>
                {active === 'custom' ? (
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium">{t('settings.appearance.accent')}</span>
