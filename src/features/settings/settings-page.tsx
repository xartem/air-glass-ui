...
export function SettingsPage() {
  ...
  const setPrivacyCategories = useCallback(
    (categories: string | null) =>
      setPrivacyOverrides((current) => ({ ...current, categories })),
    [],
  );
  const setPrivacyTexts = useCallback(
    (texts: string | null) =>
      setPrivacyOverrides((current) => ({ ...current, texts })),
    [],
  );
  ...
  const globalQuality = useMemo(() => {
    const media = settingsQuery.data?.groups.find(
  ...
  const beforeSaveCode = useCallback(
    (changed: Record<string, SettingValue>): Promise<boolean> => {
      const codeKeys = Object.keys(changed).filter((key) =>
        key.startsWith("code_"),
      );
      if (codeKeys.length === 0) return Promise.resolve(true);
      const settings = queryClient.getQueryData<{
        groups: { group: string; values: Record<string, SettingValue> }[];
      }>(["settings"]);
      const initial =
        settings?.groups.find((group) => group.group === "code")?.values ?? {};
      setCodeDiffs(
        codeKeys.map((key) => ({
          key,
    ...
    tab === "privacy"
      ? {
          ...(privacyOverrides.categories !== null
            ...
            : {}),
          ...(privacyOverrides.texts !== null
            ? { "privacy.consent_texts": privacyOverrides.texts }
            : {}),
        }
      : {};
      ...
      <div className="space-y-4 pb-24">
        <SettingsGroupForm
          ...
          visibleWhen={
            tab === "privacy"
              ...
              : tab === "media"
                ? (key, values) =>
                    !key.startsWith("media.s3.") ||
                    values["media.driver"] === "s3"
                : undefined
          }
          extraChanged={
          ...
          aside={
            tab === "media" ? (
              <MediaRegenStatus
                job={regenJob}
                onDone={() => setRegenJob(null)}
              />
            ) : tab === "privacy" ? (
              <PrivacyCategories
                onOverridesChange={setPrivacyCategories}
        ...
        {tab === "media" ? (
          <MediaPresetsTable
            ...
            onRegenerateGroup={(group, count) =>
              setRegenConfirm({ group, count })
            }
          />
        ) : null}
        ...
        {tab === "privacy" ? (
          <PrivacyTexts
            onOverridesChange={setPrivacyTexts}
            resetKey={privacyResetKey}
          />
        ) : null}
      </div>
      ...
      >
        <DialogContent className="max-w-lg">
              ...
              <div key={diff.key} className="space-y-1">
                ...
                <pre className="overflow-x-auto rounded-lg border bg-muted/40 p-2 font-mono text-xs leading-5">
                  ...
                  {diff.added.map((line, index) => (
                    <div
                      key={`a${index}`}
                      className="bg-[var(--status-success-bg)] text-[var(--status-success-fg)]"
                    >
                      + {line}
                    </div>
                  ))}
                  {diff.removed.length === 0 && diff.added.length === 0 ? (
                    <div className="text-muted-foreground">
                      {t("settings.code.no_visible_diff")}
                    </div>
                  ) : null}
                </pre>
              </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={() => resolveCodeConfirm(true)}>
              {t("settings.code.confirm_save")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
