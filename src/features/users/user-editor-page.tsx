...
export function UserEditorPage() {
  ...
  const initial = useMemo<FormState>(() => {
    if (isNew) return { name: '', email: '', role_id: null, ui_locale: 'ru', is_active: true, password: '' }
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      ...
      return isNew ? api.users.create(payload) : api.users.update(userId!, payload)
    },
    ...
    onError: (cause) => {
      if (cause instanceof ValidationError) {
        setErrors(Object.fromEntries(Object.entries(cause.fields).map(([key, code]) => [key, t(`users.error.${code}`)])))
      } else {
  ...
  const formBody = loading ? (
    ...
    <Panel icon={UserCog} title={t('users.section.main')}>
      <div className="grid gap-4 md:grid-cols-2">
        <FormField name="name" label={t('users.field.name')} required error={errors.name} className="md:col-span-2">
          <Input id="name" value={values.name} onChange={(event) => set('name', event.target.value)} />
        </FormField>
        <FormField name="email" label={t('users.field.email')} required error={errors.email} className="md:col-span-2">
          <Input id="email" type="email" value={values.email} onChange={(event) => set('email', event.target.value)} />
        </FormField>

        <FormField name="role_id" label={t('users.field.role')} required error={errors.role_id}>
        ...
        <FormField name="ui_locale" label={t('users.field.ui_locale')}>
          <Select value={values.ui_locale} onValueChange={(value) => set('ui_locale', value)}>
        ...
        {isNew ? (
          <FormField name="password" label={t('users.field.password')} required error={errors.password} className="md:col-span-2">
          ...
          <FormField name="is_active" label={t('users.field.active')} className="md:col-span-2">
            <SelfLockTooltip locked={isSelf}>
              <Switch
                id="is_active"
  ...
  const securityBody =
    !isNew && !loading && userQuery.data?.mfa_enabled && !isSelf ? (
      <Panel icon={ShieldOff} title={t('users.mfa_reset.section')} description={t('users.mfa_reset.hint')}>
        <Button type="button" variant="outline" onClick={() => setMfaResetOpen(true)}>
...
function SelfLockTooltip({ locked, children }: { locked: boolean; children: ReactNode }) {
