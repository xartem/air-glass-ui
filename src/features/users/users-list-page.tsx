...
import { api, ValidationError, type RoleDetail, type UserListItem } from '@/api'
...
export function UsersListPage() {
  ...
  const [activeTarget, setActiveTarget] = useState<{ user: UserListItem; next: boolean } | null>(null)
  ...
  const filters = {
    ...
    active: (searchParams.get('active') as 'active' | 'inactive' | null) ?? undefined,
    sort: (searchParams.get('sort') as 'name' | 'role' | 'last_login' | null) ?? 'last_login',
  ...
  const rolesQuery = useQuery({ queryKey: ['roles'], queryFn: api.roles.all, staleTime: 30_000 })
  ...
  const passwordMutation = useMutation({
    mutationFn: ({ id, password }: { id: number; password: string }) => api.users.update(id, { password }),
    onSuccess: () => {
  ...
  const activeMutation = useMutation({
    ...
    onSuccess: (_data, variables) => {
      toast.success(t(variables.next ? 'users.activated' : 'users.deactivated'))
      void queryClient.invalidateQueries({ queryKey: ['users'] })
    },
    onError: (cause) => {
      // Server invariants (self / last active admin) surface as 422 codes.
      const code = cause instanceof ValidationError ? cause.fields._error : undefined
  ...
  const columns = useMemo<ColumnDef<UserListItem>[]>(
    () => [
          ...
          return (
            <div className="min-w-0">
              <div className="flex items-center gap-2 font-medium">
                ...
                {user.is_self ? <span className="text-xs text-muted-foreground">{t('users.you')}</span> : null}
                <span className="md:hidden">
                  {!user.is_active ? <StatusBadge status="archived" label={t('users.inactive')} /> : null}
                </span>
              </div>
      ...
      },
      {
        ...
        meta: { sortable: true },
        cell: ({ row }) => <span className="whitespace-nowrap">{row.original.role.label}</span>,
      },
      {
        id: 'active',
        ...
        cell: ({ row }) => {
          ...
          return (
            <span className="whitespace-nowrap text-muted-foreground">
              {iso ? formatDistanceToNow(new Date(iso), { addSuffix: true, locale: DATE_LOCALES[locale] }) : t('users.never')}
            </span>
          )
        },
  ...
  const rowActions: RowAction<UserListItem>[] = [
    ...
    },
    {
      ...
      disabledHint: (user) =>
        user.is_self ? t('users.deactivate.self_hint') : t('users.deactivate.last_admin_hint'),
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        icon={Users}
        helpKey="users.list"
        primaryAction={{ label: t('users.add'), href: '/users/new', permission: 'users.manage' }}
      />
        ...
        actions={
          <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            <Select
              ...
              onValueChange={(value) => patchParams({ role: value === 'all' ? undefined : value })}
            >
              <SelectTrigger className="w-40">
                <SelectValue placeholder={t('users.filter.role')} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t('users.filter.all_roles')}</SelectItem>
            ...
            <Select
              ...
              onValueChange={(value) => patchParams({ active: value === 'all' ? undefined : value })}
            >
              <SelectTrigger className="w-36">
                <SelectValue placeholder={t('users.filter.active')} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t('users.filter.all_active')}</SelectItem>
      ...
      >
        <DataTable<UserListItem>
          ...
          emptyState={{ title: t('users.empty'), description: t('users.empty_hint') }}
        />
      </Panel>
      ...
      <ConfirmDialog
        ...
        title={t('users.deactivate.title', { name: activeTarget?.user.name ?? '' })}
        description={t('users.deactivate.description')}
        confirmLabel={t('users.deactivate.action')}
        destructive
        onConfirm={() => {
          if (activeTarget) activeMutation.mutate({ id: activeTarget.user.id, next: false })
      ...
      <ConfirmDialog
        ...
        title={t('users.impersonate.title', { name: impersonateTarget?.name ?? '' })}
