...
import { LayoutDashboard, MoreHorizontal, Pencil, ShieldCheck, Trash2 } from 'lucide-react'
...
export function RolesPage() {
  ...
  const [roles, permissions] = useQueries({
    queries: [
      ...
      { queryKey: ['roles', 'permissions'], queryFn: api.roles.permissions, staleTime: 5 * 60_000 },
    ],
  })
  ...
  const value = useMemo<RolePermissionMap>(() => {
    ...
    for (const role of roleList) merged[role.id] = edits[role.id] ?? initial[role.id] ?? []
    return merged
  }, [roleList, edits, initial])

  const changedRoleIds = useMemo(
    () => Object.keys(edits).map(Number).filter((id) => sortedKey(edits[id]) !== sortedKey(initial[id] ?? [])),
    [edits, initial],
  )
  const dirty = changedRoleIds.length > 0

  const saveMutation = useMutation({
    mutationFn: () => Promise.all(changedRoleIds.map((id) => api.roles.savePermissions(id, value[id]))),
    onSuccess: async () => {
      setEdits({})
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title={t('nav.roles')}
        icon={ShieldCheck}
        primaryAction={{ label: t('roles.add'), onClick: () => setCreateOpen(true), permission: 'roles.manage' }}
      />

      <Panel icon={ShieldCheck} title={t('nav.roles')} description={t('roles.hint')} contentClassName="p-0">
        {loading ? (
          ...
          <RoleMatrix
            ...
            value={value}
            onChange={(roleId, keys) => setEdits((current) => ({ ...current, [roleId]: keys }))}
            renderRoleMenu={(role) => {
              ...
              return (
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    {!role.is_system ? (
                      <>
                        <DropdownMenuItem onSelect={() => setRenameTarget(role)}>
                          <Pencil />
                          {t('roles.rename')}
                        </DropdownMenuItem>
      ...
      <CreateRoleDialog
        ...
        roles={roleList}
        onCreated={() => void queryClient.invalidateQueries({ queryKey: ['roles'] })}
      />

      <RenameRoleDialog
        role={renameTarget}
        onOpenChange={(open) => !open && setRenameTarget(null)}
        onRenamed={() => void queryClient.invalidateQueries({ queryKey: ['roles'] })}
      />
      ...
      <Dialog open={blockedRole !== null} onOpenChange={(open) => !open && setBlockedRole(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{t('roles.delete_blocked_title')}</DialogTitle>
            <DialogDescription>
              {t('roles.delete_has_users', { count: blockedRole?.users_count ?? 0 })}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button asChild onClick={() => setBlockedRole(null)}>
              <Link to={`/users?role=${blockedRole?.key ?? ''}`}>{t('roles.delete_reassign')}</Link>
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
function CreateRoleDialog({
  ...
  const mutation = useMutation({
    mutationFn: () =>
      api.roles.create({ key, label, copy_from_id: copyFrom === 'none' ? undefined : Number(copyFrom) }),
    onSuccess: () => {
    ...
    onError: (cause) => {
      if (cause instanceof ValidationError) {
        setErrors(Object.fromEntries(Object.entries(cause.fields).map(([field, code]) => [field, t(`roles.error.${code}`)])))
      } else {
        toast.error(t('common.request_failed'))
      }
    },
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4">
          <FormField name="role-key" label={t('roles.field.key')} required help={t('roles.field.key_hint')} error={errors.key}>
            <Input
              id="role-key"
              value={key}
              className="font-mono"
          ...
          <FormField name="role-label" label={t('roles.field.label')} required error={errors.label}>
            <Input id="role-label" value={label} onChange={(event) => setLabel(event.target.value)} />
          </FormField>
    ...
    <Dialog open={role !== null} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>{t('roles.rename_hint', { key: role?.key ?? '' })}</DialogDescription>
        </DialogHeader>
        <FormField name="rename-label" label={t('roles.field.label')} required>
          <Input id="rename-label" value={label} onChange={(event) => setLabel(event.target.value)} />
        </FormField>
        <DialogFooter>
          <Button variant="ghost" onClick={() => onOpenChange(false)} disabled={mutation.isPending}>
            {t('common.cancel')}
          </Button>
          <Button onClick={() => mutation.mutate()} disabled={mutation.isPending || !label.trim()}>
            {t('common.save')}
          </Button>
        </DialogFooter>
      </DialogContent>
