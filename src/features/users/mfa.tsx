  ...
  if (!start) {
    return (
      <Button onClick={() => enrollMutation.mutate()} disabled={enrollMutation.isPending}>
        {enrollMutation.isPending ? <Spinner /> : <ShieldCheck className="size-4" />}
        {t('mfa.start')}
      </Button>
    )
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-start gap-4">
        ...
        <div className="min-w-0 flex-1 space-y-2">
          <p className="text-xs text-muted-foreground">{t('mfa.secret_label')}</p>
      ...
      <Button onClick={() => confirmMutation.mutate()} disabled={confirmMutation.isPending || code.trim().length === 0}>
...
export function SecurityCard() {
  ...
  return (
    <Panel icon={ShieldCheck} title={t('mfa.title')} description={t('mfa.hint')}>
        ...
        {enabled ? (
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" onClick={() => regenerateMutation.mutate()} disabled={regenerateMutation.isPending}>
              {regenerateMutation.isPending ? <Spinner /> : <KeyRound className="size-4" />}
              {t('mfa.regenerate')}
            </Button>
      ...
      <Dialog open={disableOpen} onOpenChange={setDisableOpen}>
        <DialogContent className="max-w-sm">
          ...
          <FormField name="mfa-disable-code" label={t('mfa.code_label')} error={disableError}>
            <Input
              id="mfa-disable-code"
              value={disableCode}
              onChange={(event) => {
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={disableMutation.isPending || disableCode.trim().length === 0}
            >
      ...
      <Dialog open={regenerated !== null} onOpenChange={(open) => !open && setRegenerated(null)}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            <Button onClick={() => setRegenerated(null)}>{t('mfa.done')}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </Panel>
