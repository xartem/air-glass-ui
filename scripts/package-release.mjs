...
export const SOURCE_EXCLUDE = ["**/changelog.txt", "**/.DS_Store"];
...
const matchesAny = (rel, patterns) =>
  patterns.some((p) => globToRegExp(p).test(rel));
...
function gateStep(name, fn, { hint } = {}) {
  ...
  try {
    const note = fn();
    console.log(
      `  ${green("✓")} ${label} ${dim(`${Date.now() - started}ms`)}${note ? dim(` — ${note}`) : ""}`,
    );
    return true;
  } catch (error) {
...
export function productMetrics() {
  const locales = readdirSync(path.join(repoRoot(), "src/locales")).filter(
    (f) => f.endsWith(".json"),
  );
  const uiPrimitives = readdirSync(
    path.join(repoRoot(), "src/components/ui"),
  ).filter((f) => f.endsWith(".tsx"));
  const nav = readFileSync(path.join(repoRoot(), "src/app/nav.ts"), "utf8");
  // Not anchored to line start: prettier collapses short entries to
  // `{ to: "/", label: … }`, and an anchored match silently drops all 18 of them.
  const navLinks = (nav.match(/\bto:\s*"/g) ?? []).length;
  return {
...
function assertCleanShippedCode() {
  const offenders = [];
  const files = listFiles(path.join(repoRoot(), "src")).filter((f) =>
    /\.(ts|tsx)$/.test(f),
  );
  for (const rel of files) {
    ...
    text.split("\n").forEach((line, i) => {
      ...
      if (/(?<![\w.])console\.\w+\(/.test(line))
        offenders.push(`${at}  console.* in shipped code`);
      if (/\b(TODO|FIXME|XXX|HACK)\b/.test(line))
        offenders.push(`${at}  ${line.trim().slice(0, 60)}`);
      // CMS-era spec pointers: meaningless to a buyer reading the source.
      if (/(?:^|\s)(?:E[1-6]|D:|UI:)\s*§|§\s*\d/.test(line))
        offenders.push(`${at}  spec reference`);
    });
  }
  if (offenders.length) {
    throw new Error(
...
/**
 * The archive ships `dist/` as the buyer's zero-backend demo. That only works
 * if the mock layer was compiled in, which happens under VITE_DEMO=1 alone.
 * Assert it against the emitted bundle rather than trusting the build command:
 * a plain `npm run build` emits no mock chunk at all, and the resulting demo
 * 404s on every request.
 */
function assertDemoMockBundled() {
  const assets = path.join(repoRoot(), "dist/assets");
  const chunks = readdirSync(assets).filter((f) => f.endsWith(".js"));
  const carrier = chunks.find((f) =>
    readFileSync(path.join(assets, f), "utf8").includes("mock.session"),
  );
  if (!carrier) {
    throw new Error(
      "no bundled chunk references the mock layer — dist/ was built without " +
        "VITE_DEMO=1 and would 404 against /console/api/*",
    );
  }
  return `mock bundled in ${carrier}`;
}
...
function assertDocsFresh() {
  ...
  const docs = [
    ...
    [
      ...
      path.join(repoRoot(), "documentation/index.html"),
    ],
  ];
  ...
  for (const [label, file] of docs) {
    ...
    if (/\(en,\s*de/.test(text)) {
      const missing = m.localeCodes.filter(
        (c) => !new RegExp(`[(,]\\s*${c}\\b`).test(text),
      );
      if (missing.length)
        problems.push(`${label}: locale list omits ${missing.join(", ")}`);
    }
    for (const match of text.matchAll(
      /(\d+)\+?\s+(?:accessible\s+)?(?:UI\s+)?components/gi,
    )) {
      const stated = Number(match[1]);
      if (stated > m.uiPrimitives) {
        problems.push(
          `${label}: claims "${match[0]}", only ${m.uiPrimitives} primitives exist`,
        );
      }
    }
  }
...
function assertBundleBudget() {
  ...
  for (const ref of refs) {
    ...
    if (size / 1024 > BUDGET.singleChunkKb)
      heavy.push(`${path.basename(file)} ${(size / 1024).toFixed(0)} KB`);
  }
  ...
  if (totalKb > BUDGET.entryTotalKb) {
    problems.push(
      `entry graph ${totalKb.toFixed(0)} KB gzip exceeds ${BUDGET.entryTotalKb} KB`,
    );
  }
  if (heavy.length)
    problems.push(
      `eager chunk over ${BUDGET.singleChunkKb} KB: ${heavy.join(", ")}`,
    );
  if (problems.length) throw new Error(problems.join("\n"));
...
function runGate() {
  ...
  gateStep("format (prettier)", () => npmRun("format:check"), {
    hint: "npm run format",
  });
  gateStep("RTL logical properties", () => npmRun("lint:rtl:all"), {
    hint: "npm run lint:rtl:all",
  });
  gateStep("tests (vitest)", () => npmRun("test"), { hint: "npm test" });
  // VITE_DEMO=1 is not optional here. The archive ships this `dist/` as the
  // buyer's ready-to-open demo, and the mock API is compiled in only under that
  // flag (see `useMock` in src/api/client.ts). Built without it, every screen
  // requests /console/api/* against a backend that does not exist, 404s, and
  // bounces to the login page — a dead template in the buyer's hands.
  gateStep(
    "production build (demo)",
    () => npmRun("build", { VITE_DEMO: "1" }),
    {
      hint: "VITE_DEMO=1 npm run build",
    },
  );
  gateStep("demo mock is bundled", assertDemoMockBundled, {
    hint: "Build with VITE_DEMO=1 so the shipped dist/ runs with no backend.",
  });
  ...
  gateStep("locale key parity", assertLocaleParity, {
    hint: "Add the new keys to every locale.",
  });
...
function assemble(version) {
  ...
  try {
    execFileSync("zip", ["-rq", archive, name], {
      cwd: staging,
      stdio: ["ignore", "pipe", "pipe"],
    });
  } catch (error) {
  ...
  const contents = execFileSync("unzip", ["-Z1", archive], { encoding: "utf8" })
    .split("\n")
    .filter(Boolean);
  rmSync(staging, { recursive: true, force: true });
...
function verifyArchive(contents) {
  ...
  for (const entry of contents) {
    ...
    if (/(?:^|\/)changelog\.txt$/.test(rel))
      strays.push(`${entry}  (dev changelog)`);
    if (/^[^/]+\.(png|jpe?g)$/.test(rel))
      strays.push(`${entry}  (root-level capture)`);
  }
  if (strays.length) {
  ...
  console.log(
    `  ${green("✓")} no forbidden entries ${dim(`(${contents.length} entries checked)`)}`,
  );
  return true;
}
...
function main() {
  ...
  if (skipGate && !override) {
    ...
    console.error(
      "If that is genuinely what you want, add --i-know-what-im-doing.\n",
    );
    process.exit(2);
  }
  ...
  if (skipGate) {
    ...
    heading("Result");
    console.log(
      `${red("✗")} ${failures.length} gate failure(s) — no archive was produced.\n`,
    );
  ...
  if (dryRun) {
    heading("Result");
    console.log(
      `${green("✓")} gate passed. ${dim("Dry run — no archive written.")}\n`,
    );
    return;
  }
  ...
  if (unmatched.length) {
    ...
    console.log(
      dim(
        "      A rule that matches nothing usually means a renamed or deleted path.",
      ),
    );
  }
  ...
  if (!ok) {
    console.log(
      `${red("✗")} archive rejected — ${archive} was written but must not be shipped.\n`,
    );
    process.exit(1);
  }
  ...
  console.log(
    `${green("✓")} ${path.basename(archive)}  ${dim(`${(bytes / 1024 / 1024).toFixed(1)} MB · ${contents.length} entries`)}`,
  );
  ...
  console.log(
    `  ${dim("against Envato Author Help before uploading — they change.")}\n`,
  );
}
